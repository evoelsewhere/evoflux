...
#
#     "EvoFlux.app" is damaged and can't be opened. You should
...
#   2. Move it to ``~/.local/bin/EvoFlux`` (user-local, no sudo).
...
#     ./install.sh /path/to/EvoFlux.app           # macOS
#     ./install.sh /path/to/EvoFlux.AppImage      # Linux
#
...
if [ "$PLATFORM" = "macos" ]; then
  ...
  if [ -z "$BUNDLE" ] || [ ! -d "$BUNDLE" ] || [ "${BUNDLE##*.}" != "app" ]; then
    fail "Cannot find EvoFlux.app. Pass the path explicitly:"
    fail "    $0 /path/to/EvoFlux.app"
    exit 1
  fi
...
PLIST
  ...
  ok "${BOLD}Done.${RESET} You can launch EvoFlux now."
  exit 0
fi
...
if [ "$PLATFORM" = "linux" ]; then
  ...
  if [ -z "$BINARY" ]; then
    for candidate in \
      "$script_dir"/EvoFlux*.AppImage \
      "$script_dir"/../EvoFlux*.AppImage \
      "$PWD"/EvoFlux*.AppImage \
      "$script_dir/EvoFlux" \
      "$script_dir/../EvoFlux"; do
  ...
  if [ -z "$BINARY" ] || [ ! -f "$BINARY" ]; then
    fail "Cannot find EvoFlux binary / AppImage. Pass the path explicitly:"
    fail "    $0 /path/to/EvoFlux.AppImage"
    exit 1
  fi
  ...
  mkdir -p "$apps_dir"
  desktop_file="$apps_dir/EvoFlux.desktop"
...
DESKTOP
  ...
  for size in 32 64 128 256 512; do
    for src in \
      ...
      if [ -f "$src" ]; then
        ...
        mkdir -p "$target_dir"
        cp -f "$src" "$target_dir/EvoFlux.png"
      fi
    done
  done
  ...
  case ":$PATH:" in
    *":$bin_dir:"*)
      ok "${BOLD}Done.${RESET} Launch with: ${BOLD}EvoFlux${RESET}"
      ;;
    *)
