...
# Build the Python sidecar bundle
...
Desktop packages do not embed third-party document applications and require no
document-runtime URL/checksum secrets. DOCX and PDF authoring use the Python
dependencies already present in the sidecar. Optional Office preview/rendering
uses explicitly configured or host-installed LibreOffice and Poppler; optional
Node pipelines similarly resolve Node, Chromium, and artifact-tool from the
active workspace or explicit environment overrides. A missing optional tool
disables only the feature that needs it and does not prevent desktop startup.

For controlled deployments that already have redistribution rights, a verified
runtime can still be supplied with `--document-runtime` or
`EVOFLUX_DOCUMENT_RUNTIME_SOURCE`. It remains an opt-in compatibility path and
is not used by the release workflow.
