...
const workspaceTreeResponse = () => ({
  repositories: Array.from(
    new Set(
      sessionsData
        .filter((session) => session.mode === "coding" && session.workspace)
        .map((session) => session.workspace as string),
    ),
  ).map((path) => ({
    path,
...
mock.module("@/hooks/use-platform", () => ({
  usePlatform: () => ({
    isTauri,
    os: platformOs,
    isMacOverlay: isTauri && platformOs === "macos",
  }),
  getPlatform: () => ({
    isTauri,
    os: platformOs,
...
mock.module("framer-motion", () => ({
  ...
  motion: {
    aside: ({
      ...
      void transition;
      return (
        <aside data-animate={JSON.stringify(animate)} {...props}>
          {children}
        </aside>
      );
    },
...
describe("CodingSidebar workspace trust flow", () => {
  beforeEach(() => {
    ...
    globalThis.fetch = mock(async (input: unknown) => {
      ...
      if (url.includes("/api/team/workspace/validate")) {
        if (validateError) {
          return new Response(
            JSON.stringify({ detail: validateError.message }),
            { status: 422 },
          );
        }
  ...
  async function renderCodingSidebarWithProps(
    props: React.ComponentProps<
      typeof import("@/components/CodingSidebar").CodingSidebar
    >,
  ) {
    const { CodingSidebar } = await import("@/components/CodingSidebar");
    const queryClient = new QueryClient();
  ...
  it("keeps running sessions visible when a workspace is collapsed", async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByLabelText("Collapse repository project"));

    expect(screen.getByLabelText("Expand repository project")).toBeTruthy();
    expect(screen.getByText("Background running session")).toBeTruthy();
    expect(
      screen.getByLabelText("Repository has running session"),
    ).toBeTruthy();
    expect(screen.getByLabelText("Session running")).toBeTruthy();
  });
  ...
  it("hides a main repository from the sidebar without deleting it", async () => {
    ...
    await user.click(
      screen.getByLabelText("Hide repository project from sidebar"),
    );
    expect(screen.getByText("Remove workspace from sidebar")).toBeTruthy();
    await user.click(
      screen.getByRole("button", { name: /^remove from sidebar$/i }),
    );
  ...
  it("does not create a new session when the current coding session is empty and idle", async () => {
    ...
    await renderCodingSidebarForSessions("session-1");
    await user.click(
      screen.getByLabelText("New session in main workspace project"),
    );
            ...
            repositories: [
              {
                ...
                worktrees: [
                  {
                    path: "/data/worktrees/project/task-a",
                    name: "task-a",
                    managed: true,
                  },
                ],
  ...
  it("renders managed worktrees under their source repository with distinct actions", async () => {
    ...
    expect(screen.getByLabelText("Collapse repository project")).toBeTruthy();
    expect(
      screen.getByLabelText("Collapse main workspace project"),
    ).toBeTruthy();
    ...
    expect(useTeamStore.getState()._workspace).toBe(
      "/data/worktrees/project/task-a",
    );
    expect(navigate).toHaveBeenCalledWith({
      to: "/coding/$sessionId",
