 ...
 *
 * ``shell`` / ``patch`` always invalidate the workspace regardless
 * of args. ``write`` / ``edit`` / ``rm`` to a
...
describe("useTeamStore — coding_workspace invalidation", () => {
  ...
  it("emits broad coding_workspace event when `bg` finishes in coding mode (no extractable path)", () => {
      // ``bg`` is shell-like (arbitrary path mutations) and doesn't surface
      // deterministic output paths in its args. Falls back to broad invalidation.
      resetStore({ sessionId: "sess-c3", _workspace: "/tmp/proj" })
      primeBlock("claude", "bg", "tc-bg", { foo: "bar" })
      useTeamStore.getState()._handleSSEEvent("tool_end", {
        name: "bg",
        agent: "claude",
        tool_call_id: "tc-bg",
