...
const DISMISSED_UNTIL_KEY = 'EvoFlux.updater.dismissedUntil'
...
function ReleaseNotesButton({ fallbackNotes, version }: { fallbackNotes?: string | null; version: string }) {
  ...
  return (
    <>
      ...
      {open ? (
        <div className="mobile-safe-overlay fixed inset-0 z-60 flex items-center justify-center bg-(--color-overlay) p-4" role="dialog" aria-modal="true" aria-label="Release notes" onClick={() => setOpen(false)}>
...
function titleForStatus(status: UpdateStatus): string {
  ...
  if (status.status === 'available') return `EvoFlux ${status.version} is available`
  if (status.status === 'downloading') return `Downloading EvoFlux ${status.version}`
  if (status.status === 'downloaded') return `EvoFlux ${status.version} is ready to install`
  if (status.status === 'installing') return 'Installing update...'
  if (status.status === 'up_to_date') return 'EvoFlux is up to date'
...
function descriptionForStatus(status: UpdateStatus): string {
  ...
  if (status.status === 'installing') return 'EvoFlux will restart when installation completes.'
  if (status.status === 'downloading') return 'You can keep using EvoFlux while the update downloads.'
  return ''
}
