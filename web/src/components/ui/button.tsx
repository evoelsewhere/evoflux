  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-md border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap text-(--color-text) transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-(--motion-fast) outline-none select-none focus-visible:ring-2 focus-visible:ring-(--focus-ring)/40 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-(--color-error) aria-invalid:ring-2 aria-invalid:ring-(--color-error)/20 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        gradient: "border-transparent bg-gradient-primary text-(--color-text-on-accent) hover:opacity-90 active:opacity-80 shadow-[0_2px_8px_rgba(102,126,234,0.35)]",
        outline:
        ...
        destructive:
          "border-(--color-error)/30 bg-(--color-error-subtle) text-(--color-error) hover:bg-(--color-error)/15 focus-visible:ring-(--color-error)/20",
        link: "bg-transparent text-(--accent-blue-text) underline-offset-4 hover:underline",
      },
      size: {
        default:
          "h-9 gap-1.5 px-2.5 md:h-8 has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2",
        xs: "h-6 gap-1 rounded-sm px-1.5 text-xs has-data-[icon=inline-end]:pr-1 has-data-[icon=inline-start]:pl-1 [&_svg:not([class*='size-'])]:size-3",
        sm: "h-8 gap-1 rounded-sm px-2 text-[0.8rem] md:h-7 has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5 [&_svg:not([class*='size-'])]:size-3.5",
        lg: "h-10 gap-1.5 px-3 md:h-9 has-data-[icon=inline-end]:pr-2.5 has-data-[icon=inline-start]:pl-2.5",
        icon: "size-9 md:size-8",
        "icon-xs":
          "size-6 rounded-sm [&_svg:not([class*='size-'])]:size-3",
        "icon-sm":
          "size-8 rounded-sm md:size-7",
