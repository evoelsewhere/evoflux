...
function Tabs({
  ...
  return (
    <TabsPrimitive.Root
      ...
      className={cn("group/tabs flex gap-1.5 data-horizontal:flex-col", className)}
      {...props}
    />
  )
}
...
const tabsListVariants = cva(
  "group/tabs-list inline-flex w-fit max-w-full items-center justify-center overflow-x-auto rounded-md border border-(--color-border) bg-(--bg-page) p-0.5 text-(--color-text-muted) scrollbar-none group-data-horizontal/tabs:h-8 group-data-vertical/tabs:h-fit group-data-vertical/tabs:flex-col group-data-vertical/tabs:overflow-x-visible data-[variant=line]:border-transparent data-[variant=line]:bg-transparent data-[variant=line]:p-0",
  {
    variants: {
      variant: {
        default: "",
