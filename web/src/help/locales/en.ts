...
export const HELP_ARTICLES_EN: HelpArticle[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'When an agent creates a Word, Excel or PowerPoint file in a Work session, Files opens its preview while the turn runs (unless you are using another workbench tool) and redraws it on every save. A deck built with the PowerPoint Skill fills in slide by slide in the session building it, while its agent works (other sessions and later opens show the slides that exist): finished slides appear as they land, the slide being built shows an animated loading skeleton, and later slides are listed as Up next. While it is being built the slide thumbnails and the file tree step aside and the viewer follows the newest slide; scroll, click or navigate to take over, and scroll back to the slide being built to follow it again. Both come back when the deck is finished, and the exact renderer takes over from there. A workbook built with the Excel Skill fills in the same way, sheet by sheet, with a grid skeleton for each sheet still to come. Each finished turn also ends with thumbnail cards for the decks, sheets, documents and PDFs it created or changed, plus any image the reply links (documents first, four at a time, the rest under +N more); click one to preview it in Files.',
      },
      {
        type: 'p',
