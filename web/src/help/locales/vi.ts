...
export const HELP_ARTICLES_VI: HelpArticle[] = [
  ...
  },
  {
    ...
    tricks: [
      ...
      'Operational Run, mission, evidence, events, Recovery và Realtime state nằm trong `.evoflux/easd/.local/` bị ignore; các Git worktree dùng chung runtime của source repository. Accepted Specs và docs được adopt vẫn track được. Setup hiển thị exact legacy paths, revalidate generated defaults và rollback khi move lỗi.',
      'Run đã converged có thể publish thủ công một compact Git-visible audit record. EvoFlux loại prompts, raw evidence, transcripts và local paths; đồng thời không tự commit hay xóa local Recovery ledger.',
      ...
      'Mở Recovery để preview retry an toàn. UI hiển thị phase transition, Spec/Plan/session được reuse và history được preserve trước confirmation. Run trước implementation có thể được tiếp tục rõ ràng trong Coding session hiện tại tương thích; Run đang implementation hoặc đã converge không thể rebind. Repository generation stale sẽ chặn retry.',
