...
export const HELP_ARTICLES_JA: HelpArticle[] = [
  ...
  },
  {
    ...
    summary:
      'Evo Agent Specs でバージョン化された仕様を承認し、エージェントミッションを受け入れ条件に結び付け、コードスナップショットに紐づく証拠と逸脱を管理し、すべてのゲートを満たした場合だけ収束させます。',
    keywords: [
      'EASD',
      'Evo Agent Specs',
      'specification',
    ...
    tricks: [
      ...
      'Operational Run、mission、evidence、event、Recovery、Realtime state は ignored `.evoflux/easd/.local/` に保存し、linked Git worktree は source repository runtime を共有します。Accepted Spec と adopted docs は track 可能です。Setup は exact legacy path を preview し、generated default を再検証して move failure を rollback します。',
      'Converged Run は compact Git-visible audit record を手動 publish できます。EvoFlux は prompt、raw evidence、transcript、local path を除外し、local Recovery ledger を自動 commit/delete しません。',
      ...
      'Recovery では安全な retry を事前確認できます。確認前に phase transition、再利用する Spec/Plan/session identity、保持する履歴を表示します。Repository generation が stale なら retry を停止し、converged Run は immutable のままです。',
      ...
      '仕様範囲の変化は暗黙に実装せず deviation として記録します。',
      'Converge は Verify phase の run だけを受け付ける server gate であり、agent confidence claim ではありません。',
    ],
    blocks: [
      {
        type: 'p',
        text: 'EASD は Evo Agent Specification-Driven Development の略です。SDD が規範契約を定義し、ADD が境界付きエージェントミッションとして実行する、EvoFlux の仕様統治型 Agent-Driven Development プロトコルです。',
      },
      {
        type: 'p',
      ...
      {
        type: 'p',
        text: 'Project Skill は Specify、flow-dependent Plan、Implement、Review、Verify を案内しますが lifecycle authority は付与しません。User が Spec、planned 時の Plan、後続 phase、Converge を明示的に操作します。',
      },
      {
        type: 'p',
        text: 'Intent を作成し、lead に specification と flow recommendation を draft させて Approve します。Direct は Plan artifact なしで implementation へ進み、planned は acyclic mission graph を別途 approve します。両方とも明示的な Review と Verify が必須です。',
      },
      {
        type: 'tips',
        items: [
          'Draft — 編集可能な仕様。',
          'Accepted revision — 不変 payload と SHA-256 hash。',
          'Accepted plan — planned flow 用の optional immutable mission graph。',
          'Mission — AC に紐づく永続 DelegationTask。',
          'Evidence — machine、review、manual、waiver の出所。',
          'Deviation — 可視化された契約・範囲の逸脱。',
          'Convergence — 必須 AC と mission の全ゲートを満たした状態。',
        ],
      },
      {
        type: 'p',
        text: 'Cross-layer と critical の run では独立 review evidence も必要です。未実行の planned verification command、blocking deviation、未達 AC、失敗 evidence、未完了 mission があれば、偽の Done ではなく構造化された理由を返します。',
      },
      {
        type: 'p',
        text: 'Panel と public API は manual、review、waiver evidence を追加できますが、machine evidence や runtime reviewer independence は生成できません。Trusted runtime tool がその trust level を所有します。',
      },
      {
        type: 'p',
        text: 'よくある誤り：required Plan の省略、direct flow への Plan identity 捏造、Spec/AC/scope identity なしの委譲、review を convergence とみなすこと、隠れた scope expansion。',
      },
    ],
