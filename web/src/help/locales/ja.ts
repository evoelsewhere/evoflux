...
export const HELP_ARTICLES_JA: HelpArticle[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'モード切替の手順: (1) 必要ならサイドバーを開く、(2) モードスイッチャーで Work / Coding をクリック、(3) セッションを選ぶか作成、(4) プロンプト前に、そのモードで必要な workbench ツールが使えることを確認。',
      }
],
    related: [
      'coding-workspaces',
  ...
  },
  {
    ...
    tricks: [
      ...
      '長いトランスクリプトは上端の数画面前から古い履歴を先読みし、上へ高速スクロールしても古いターン追加時に読んでいる位置を保ちます。',
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'Office ファイル（.docx / .xlsx / .pptx）は読み取り専用ビューアで開きます。内蔵レンダラーのレイアウトは近似です。正確なページが必要なら、ビューアの Install renderer を押してオプションの LibreOffice レンダラー（約 150〜190 MB、Windows と macOS で一度だけ）をダウンロードしてください。依頼するまで何もダウンロードされません。アーカイブは固定されたチェックサムで検証されてからアプリのデータフォルダーにインストールされ、変換はオフラインでマクロ無効のまま実行されます。ファイルの初回の正確なレンダリングには最大 1 分ほどかかり、以降はキャッシュを使います。正確なページでも検索とテキスト選択は使えます。',
      },
      {
        type: 'p',
        text: 'Work セッションでエージェントが Word・Excel・PowerPoint ファイルを作成すると、ターン実行中に Files がそのプレビューを開き（別の workbench ツールを使用中の場合を除く）、保存のたびに再描画します。PowerPoint Skill で作るデッキは、作成中のセッションでエージェントが動いている間だけスライドごとに埋まっていきます（他のセッションや後から開いた場合は、できあがったスライドだけが表示されます）。完成したスライドは保存されるとすぐ表示され、作成中のスライドにはアニメーション付きの読み込みスケルトン、以降のスライドは Up next として並びます。作成中はスライドのサムネイルとファイルツリーが隠れ、ビューアは最新のスライドを追いかけます。スクロール・クリック・移動すると自分で操作でき、作成中のスライドまでスクロールし直すと再び追いかけます。デッキが完成すると両方が戻り、以降は正確なレンダラーに切り替わります。Excel Skill で作るワークブックも同じようにシートごとに埋まり、まだないシートにはグリッド状のスケルトンが表示されます。完了した各ターンの最後には、そのターンで作成・変更したデッキ、スプレッドシート、文書、PDF と、返信内でリンクされた画像のサムネイル付きカードが並びます（文書が先、表示は 4 件まで、残りは +N more）。カードをクリックすると Files でプレビューできます。',
      },
      {
        type: 'p',
        text: 'デッキの一部だけを直したいときは、ビューアの Select an area to edit を押し、スライド上のシェイプをクリックするか範囲をドラッグして選びます。変更内容を入力して Enter で送信するか、Add to batch で複数の範囲をまとめてから入力欄からまとめて送信します（Annotations: N と表示。範囲を指すだけなら空欄のままで構いません）。エージェントはその部分だけを編集し、作業中は選んだ範囲が光ります。ワークブックでは数式バーの Select cells to edit を使い、セルをクリックするかセル範囲をドラッグして選びます。ワークブックはセルのグリッドで開き、正確なレンダラーがあれば数式バーの Print layout で印刷時のページを表示できます。ビューアのヘッダーにある Undo・Redo・Version history で、セッション内の Word・Excel・PowerPoint ファイルの保存済みバージョンを行き来したり、以前のバージョンに戻したりできます。チャットや他のファイルには影響しません。',
      },
      {
        type: 'p',
  ...
  },
  {
    ...
    tricks: [
      ...
      'WebBridge を有効にした Coding チャットでは、エージェントが操作するタブのコンソール・エラー・リクエストを記録し、実ブラウザで開発サーバーをデバッグできます。Work チャットでは読み取りが必要なときだけ記録します。ページのエラーは Problems の Browser にも表示されます。'
],
    blocks: [
      {
        type: 'p',
  ...
  },
  {
    ...
    tricks: [
      ...
      '組み込みの computer-app-control Skill が、どのアプリでもエージェントを導きます。指定したアプリの中で作業し、背後のファイルを直接編集せず、クリップボードには触れず、クリックする前に画面を読み取り、各ステップの結果をアプリから読み戻して確かめてから次に進みます。Settings → Skills でオフにできます。',
    ...
    blocks: [
      ...
      },
      {
        ...
        tone: 'info',
        title: '制限',
        text: 'システムのシェルやセキュリティプロセス(macOS では Finder とシステム設定も)、EvoFlux 自身、管理者として実行中のアプリには接続できません。macOS では、コマンドやスクリプトを実行するアプリ(ターミナル、iTerm2、スクリプトエディタ、ショートカットなど)にも接続できません。一部のアプリ(UWP、ゲーム、raw input を読むアプリ)はバックグラウンドのクリックを無視するため、その場合エージェントはアクセシビリティの操作(invoke、set_value)を使います。macOS では、隠したアプリは画面の隅に 1 点だけ見える状態で置かれます。別の操作スペースにあるウィンドウは、先に現在のデスクトップへ移動してください。',
      },
      {
        type: 'tips',
        items: [
