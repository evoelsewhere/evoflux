...
export const HELP_ARTICLES_JA: HelpArticle[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'デッキの一部だけを直したいときは、ビューアの Select an area to edit を押し、スライド上のシェイプをクリックするか範囲をドラッグして選びます。変更内容を入力して Enter で送信するか、Add to batch で複数の範囲をまとめてから入力欄からまとめて送信します（Annotations: N と表示。範囲を指すだけなら空欄のままで構いません）。エージェントはその部分だけを編集し、作業中は選んだ範囲が光ります。ワークブックでは数式バーの Select cells to edit を使い、セルをクリックするかセル範囲をドラッグして選びます。ビューアのヘッダーにある Undo・Redo・Version history で、セッション内の Word・Excel・PowerPoint ファイルの保存済みバージョンを行き来したり、以前のバージョンに戻したりできます。チャットや他のファイルには影響しません。',
      },
      {
        type: 'p',
