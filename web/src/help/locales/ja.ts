...
export const HELP_ARTICLES_JA: HelpArticle[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'Office ファイル（.docx / .xlsx / .pptx）は読み取り専用ビューアで開きます。内蔵レンダラーのレイアウトは近似です。正確なページが必要なら、ビューアの Install renderer を押してオプションの LibreOffice レンダラー（約 150〜190 MB、Windows と macOS で一度だけ）をダウンロードしてください。依頼するまで何もダウンロードされません。アーカイブは固定されたチェックサムと署名で検証されてからアプリのデータフォルダーにインストールされ、変換はオフラインでマクロ無効のまま実行されます。ファイルの初回の正確なレンダリングには最大 1 分ほどかかり、以降はキャッシュを使います。正確なページでも検索とテキスト選択は使えます。',
      },
      {
        type: 'p',
