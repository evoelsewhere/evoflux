...
export const HELP_ARTICLES_JA: HelpArticle[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: 'Work セッションでエージェントが Word・Excel・PowerPoint ファイルを作成すると、ターン実行中に Files がそのプレビューを開き（別の workbench ツールを使用中の場合を除く）、保存のたびに再描画します。PowerPoint Skill で作るデッキは、作成中のセッションでエージェントが動いている間だけスライドごとに埋まっていきます（他のセッションや後から開いた場合は、できあがったスライドだけが表示されます）。完成したスライドは保存されるとすぐ表示され、作成中のスライドには予定タイトルとアニメーション付きのプレースホルダー、以降のスライドは Up next として並びます。作成中はスライドのサムネイルとファイルツリーが隠れ、ビューアは最新のスライドを追いかけます。スクロール・クリック・移動すると自分で操作でき、作成中のスライドまでスクロールし直すと再び追いかけます。デッキが完成すると両方が戻り、以降は正確なレンダラーに切り替わります。',
      },
      {
        type: 'p',
