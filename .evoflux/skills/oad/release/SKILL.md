---
name: oad/release
description: EvoFlux workflow for version bumps, release PRs, GitHub releases, and release notes.
---
...
- PR body shape:
  ...
  - Update desktop, backend, web, and lockfile versions in lockstep to <version>.
  ```
...
gh api repos/khuonghung/evoflux/pulls/<pr-number>/comments \
...
- `## Install`: structure by install **surface**, not by chronology. One labelled block per channel — desktop, CLI, Docker — so a reader picks their channel and stops. **Source of truth is this release's published asset list**, not the previous release's notes — the desktop matrix can silently skip a target. List the previous release only to copy styling/labelling, never to copy bullet contents:
  ...
  ```bash
  gh release view v<version> --repo khuonghung/evoflux | grep -E "^asset:"   # source of truth
  gh release view v<prev-version> --repo khuonghung/evoflux                  # styling reference only
  ```
  ...
  **CLI-only block** (patch/minor releases that don't change install surfaces — copy verbatim):
  ...
  # or
  pip install $1evoflux
  # or
  brew install khuonghung/tap/evoflux
  ```

  `brew install khuonghung/tap/evoflux` installs the base package only; optional extras (e.g. `EvoFlux[full]`) must be installed via `uv` or `pip`:
  ...
  **Desktop app** — download from this release (CLI + desktop ship under one tag since 1.0.9):
  ...
  - macOS Apple Silicon → `brew install --cask khuonghung/tap/evoflux` (recommended — ad-hoc signs automatically), or `EVOFLUX_*_aarch64.dmg` (run bundled `install.sh`, right-click → Open the first time).
  - Windows → `EVOFLUX_*_x64-setup.exe` (More info → Run anyway at SmartScreen).
  - Linux → `EVOFLUX_*_amd64.AppImage` (`chmod +x` and run) or the `.deb`.
  ...
  **Docker**
  ...
  ```
  docker pull ghcr.io/khuonghung/evoflux:<version>
  ```
  ````
...
- `## Upgrade`: when present, **split by install channel** — never combine channels in one prose paragraph. Each bullet is a copy-pasteable command (or "Settings → … → Install" path). Example shape:
  ...
  - **Desktop app via Homebrew** — `brew upgrade --cask EvoFlux`.
  - **CLI via uv** — `uv tool install $1evoflux`.
  - **CLI via pip** — `pip install $1evoflux`.
  - **CLI via Homebrew** — `brew upgrade EvoFlux`.
...
```bash
git fetch --tags
gh release view v<version> --repo khuonghung/evoflux | grep -E "^asset:"
gh release view v<prev-version> --repo khuonghung/evoflux
```

- Map the asset list to Install bullets:
  - `EVOFLUX_<ver>_aarch64.dmg` present → keep the macOS Apple Silicon bullet.
  - `EVOFLUX_<ver>_x64-setup.exe` present → keep the Windows bullet, else drop it.
  - `EVOFLUX_<ver>_amd64.AppImage` present → keep AppImage in the Linux bullet, else mention only the `.deb` (and drop the bullet entirely if `.deb` is also missing).
- Replace the auto-generated notes, then verify:

```bash
gh release edit v<version> --repo khuonghung/evoflux --notes-file <path-to-notes.md>
gh release view v<version> --repo khuonghung/evoflux | sed -n '/## What.s changed/,/Full changelog/p'
```
...
```bash
gh release view v<version> --repo khuonghung/evoflux | sed -n '/asset:/p;/## Install/,/Full changelog/p'
```
