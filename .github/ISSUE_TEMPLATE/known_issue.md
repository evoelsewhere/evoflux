---
name: Known issue
about: Track a confirmed product issue publicly
labels: bug, known issue
---

<!-- Please follow the project's Code of Conduct. Keep reports factual and respectful. -->

## Maintainer checklist

- [ ] This issue is confirmed or strongly reproducible.
- [ ] This is not a security vulnerability.

Security vulnerabilities must be reported privately through
[GitHub Security Advisories](https://github.com/khuonghung/evoflux/security/advisories/new),
not tracked in a public known issue.

## Summary

<!-- Short factual description of the known issue. -->

## Affected versions / surfaces

- Version or commit:
- Surface: <!-- desktop, CLI/server, web UI, coding mode, provider, docs, etc. -->

## Symptoms

<!-- What users see. -->

## Reproduction

1.
2.
3.

## Workaround

<!-- Known workaround, or "None known". -->

## Expected fix direction

<!-- Optional: suspected cause, likely files, or acceptable fix shape. -->
