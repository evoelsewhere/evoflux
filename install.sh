#!/bin/sh
# install.sh — one-command installer for evoflux on macOS / Linux.
#
# Usage:
...
#   2. Run `uv tool install $1evoflux` so `evoflux` lands on PATH.
#   3. Print next-step hint (`evoflux` auto-runs first-time setup).
#
# Design notes:
...
report() {
    ...
    if command -v evoflux >/dev/null 2>&1; then
        version="$(evoflux --version 2>/dev/null || echo unknown)"
        note "    evoflux $version"
    else
        cat >&2 <<EOF
warning: 'evoflux' is not on PATH for this shell.
         Open a new terminal, or add \$HOME/.local/bin to PATH.
EOF
    fi
    say ""
    say "Next: run ${BOLD}evoflux${RESET} to launch the server."
