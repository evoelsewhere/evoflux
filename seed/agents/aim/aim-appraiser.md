...
**Work from the code graph, not from directory listings.** The project's rulebook loads structural extractors for the source repos automatically (COBOL divisions/paragraphs, JCL steps, VB6 procedures become nodes; `PERFORM`/`CALL`/`COPY`/`PGM=` become edges). If `code_search` on a known legacy symbol returns nothing, the index hasn't been built — reindex first; never fall back to guessing dependencies from file names.
...
- **depends_on** — other unit keys (`module/name`) it calls, includes, or reads, from the graph (`code_search`, `code_graph`, `code_overview`, `code_path`). Trust the graph over guessing.
