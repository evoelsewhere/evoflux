...
def _run(
    ...
    return subprocess.run(
        ...
        capture_output=True,
        text=True,
        # The banner is UTF-8 box drawing; the Windows console codepage would
        # otherwise mangle it and make assertions on the output unreliable.
        encoding="utf-8",
        errors="replace",
        env=full_env,
...
class TestHelpFlag:
    ...
    def test_help_mentions_all_three_platforms(self):
        ...
        # Windows users at the NSIS setup executable.
        assert "macOS" in out
        assert "Linux" in out
...
class TestMissingBundle:
    ...
    def test_linux_missing_binary_exits_one(self, tmp_path: Path):
        ...
        assert proc.returncode == 1
        out = proc.stdout + proc.stderr
        assert "Cannot find EvoFlux" in out
...
class TestLinuxBranch:
    ...
    def fake_binary(self, tmp_path: Path) -> Path:
        """Create a fake but executable EvoFlux binary."""
        bin_path = tmp_path / "evoflux.AppImage"
    ...
    def test_linux_rpm_defers_to_rpm(self, tmp_path: Path):
        ...
        rpm = tmp_path / "EvoFlux-0.1.0.x86_64.rpm"
...
class TestMacosBranch:
    ...
    def fake_bundle(self, tmp_path: Path) -> Path:
        ...
        binary = bundle / "Contents" / "MacOS" / "evoflux"
        binary.write_text("")
        binary.chmod(0o755)
        return bundle
