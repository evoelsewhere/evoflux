...
def test_EVOFLUX_file_can_add_tools_and_skills(tmp_path):
    from app.agent.loader import load_team_from_dir
...
def test_EVOFLUX_builtin_and_user_capabilities_are_deduped(tmp_path):
...
def test_EVOFLUX_user_description_overrides_builtin_description(tmp_path):
...
def test_EVOFLUX_coding_lead_uses_coding_builtin_prompt(tmp_path):
    from app.agent.builtin_prompts import CODING_EVOFLUX_PROMPT
    from app.agent.loader import load_team_from_dir
    ...
    team = load_team_from_dir(d, provider_factory=factory, mode="coding")
    assert team is not None
    assert team.lead.agent.system_prompt.startswith(CODING_EVOFLUX_PROMPT)
