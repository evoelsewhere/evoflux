...
class TestAgentTeamUserMessage:
    ...
    async def test_removed_loop_command_is_not_delivered_as_chat(self, basic_team):
        team = basic_team
        team.mailbox.send = AsyncMock()
        session_id = str(uuid.uuid7())

        with pytest.raises(ContinuePreconditionError, match="has been removed") as exc:
            await team.handle_user_message("/loop legacy", session_id=session_id)
