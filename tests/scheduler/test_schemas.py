...
class TestModeWorkspace:
    def test_work_mode_default_no_workspace(self):
        body = ScheduledTaskCreate(
...
class TestLegacyModeAlias:
    """Historical default-mode names must coerce to ``work``, not 422."""
    ...
    def test_create_accepts_legacy_forge(self):
        body = ScheduledTaskCreate(
            name="t",
            mode="forge",
            schedule_type="every",
    ...
    def test_update_accepts_legacy_forge(self):
        body = ScheduledTaskUpdate(mode="forge")
        assert body.mode == "work"
