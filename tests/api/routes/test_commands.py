...
def roots(tmp_path: Path, monkeypatch):
    ...
    return project_EvoFlux, project_opencode, global_EvoFlux, global_opencode
...
async def test_list_without_workspace_ignores_project_local_commands(client, roots):
    project_EvoFlux, _project_opencode, global_EvoFlux, _global_opencode = roots
    _write(project_EvoFlux / "run.md", "---\ndescription: Local run\n---\nrun")
    _write(global_EvoFlux / "review.md", "---\ndescription: Review\n---\nreview")
...
async def test_list_returns_discovered(client, roots):
    project_EvoFlux, _project_opencode, _global_EvoFlux, _global_opencode = roots
    _write(project_EvoFlux / "commit.md", COMMIT)
    _write(
        project_EvoFlux / "git" / "push.md",
        "---\ndescription: Push.\n---\nPush body.\n",
    )

    res = await client.get(
        "/api/commands", params={"workspace": str(project_EvoFlux.parents[1])}
    )
...
async def test_render_substitutes_arguments(client, roots):
    ...
    res = await client.post(
        ...
        params={"workspace": str(project_EvoFlux.parents[1])},
...
async def test_create_subcommand_file_list_and_render_roundtrip(client, roots):
    ...
    project_EvoFlux, _project_opencode, _global_EvoFlux, _global_opencode = roots
    workspace = project_EvoFlux.parents[1]
    _write(
        project_EvoFlux / "git" / "commit.md",
...
@pytest.mark.asyncio
async def test_precedence_is_local_before_global_and_EVOFLUX_before_opencode(
    client, roots
):
    project_EvoFlux, project_opencode, global_EvoFlux, global_opencode = roots
    _write(
        global_opencode / "deploy.md",
        "---\ndescription: global opencode\n---\nglobal opencode",
    )
    _write(
        global_EvoFlux / "deploy.md",
        "---\ndescription: global EvoFlux\n---\nglobal EvoFlux",
    )
    _write(
    ...
    _write(
        project_EvoFlux / "deploy.md",
        "---\ndescription: project EvoFlux\n---\nproject EvoFlux",
    )
...
@pytest.mark.asyncio
async def test_local_opencode_wins_over_global_EvoFlux(client, roots):
    _project_EvoFlux, project_opencode, global_EvoFlux, _global_opencode = roots
    _write(
        global_EvoFlux / "review.md",
        "---\ndescription: global EvoFlux\n---\nglobal EvoFlux",
    )
    _write(
...
async def test_global_commands_are_available_for_each_workspace(
    ...
    _project_EvoFlux, _project_opencode, global_EvoFlux, _global_opencode = roots
    workspace_a, workspace_b = workspaces
    _write(
        global_EvoFlux / "review.md",
