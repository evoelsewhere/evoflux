...
class TestXdgDirs:
    ...
    def test_data_env_var_overrides(self, tmp_path, monkeypatch):
        monkeypatch.setenv("EVOFLUX_DATA_DIR", str(tmp_path))
        assert _data_dir() == tmp_path

    def test_config_env_var_overrides(self, tmp_path, monkeypatch):
        monkeypatch.setenv("EVOFLUX_CONFIG_DIR", str(tmp_path))
        assert _config_dir() == tmp_path

    def test_state_default_is_xdg_state(self, monkeypatch):
        monkeypatch.delenv("EVOFLUX_STATE_DIR", raising=False)
        assert _state_dir() == Path.home() / ".local" / "state" / "EvoFlux"

    def test_data_default_is_xdg_data(self, monkeypatch):
        monkeypatch.delenv("EVOFLUX_DATA_DIR", raising=False)
        assert _data_dir() == Path.home() / ".local" / "share" / "EvoFlux"

    def test_config_default_is_xdg_config(self, monkeypatch):
        monkeypatch.delenv("EVOFLUX_CONFIG_DIR", raising=False)
        assert _config_dir() == Path.home() / ".config" / "EvoFlux"
...
class TestPidFileIO:
    ...
    def test_write_pids_creates_parent_dirs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("EVOFLUX_STATE_DIR", str(target))
        _write_pids([42])
...
class TestCmdUpgrade:
    ...
    def test_brew_upgrade_does_not_relink_formula_without_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "brew",
                ["brew", "upgrade", "--formula", "khuonghung/tap/evoflux"],
            ),
        )
        monkeypatch.setattr(
            upgrade_mod, "_run", lambda command: run_calls.append(command) or 0
...
class TestCmdAddress:
    def test_address_prints_local_and_lan_urls(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "EvoFlux addresses" in out
...
class TestCmdHealth:
    def test_health_passes_for_reachable_ready_server(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "EvoFlux server health" in out
...
class TestCmdDoctor:
    ...
    def _healthy_env(self, monkeypatch, tmp_path):
        ...
        Doctor reads from the configured ``EVOFLUX_CONFIG_DIR`` (set to
        ``.tests/config`` by ``pytest.ini``); we redirect it to a tmp
        path here so we can drop a stub ``evoflux.md`` without polluting
        the shared test config dir.
        """
        from tests.conftest import set_EVOFLUX_dirs

        set_EVOFLUX_dirs(monkeypatch, tmp_path)
        monkeypatch.setenv("OPENAI_API_KEY", "sk-test")

        agents = tmp_path / "config" / "agents"
        agents.mkdir(parents=True)
        (agents / "evoflux.md").write_text(
            "---\nname: evoflux\nmodel: openai:gpt-5\n---\n"
        )
        return tmp_path
