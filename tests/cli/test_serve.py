...
class TestHandshakeFormat:
    def test_handshake_is_single_json_line_with_prefix(self):
        ...
        assert output.startswith("EVOFLUX_HANDSHAKE ")
        # Parsable JSON after the prefix.
        payload = json.loads(output.removeprefix("EVOFLUX_HANDSHAKE ").strip())
...
class TestDesktopTokenConfig:
    ...
    def test_generate_desktop_token_sets_env(self, monkeypatch):
        ...
        try:
            ...
            assert token
            assert os.environ["EVOFLUX_DESKTOP_TOKEN"] == token
        finally:
            if original_token is None:
                os.environ.pop("EVOFLUX_DESKTOP_TOKEN", None)
            else:
                os.environ["EVOFLUX_DESKTOP_TOKEN"] = original_token
