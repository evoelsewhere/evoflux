...
``chat_sessions.tags`` from revision 00000023). Complements
``tests/core/test_db_extra.py``, which only covers ``run_migrations`` error
paths with mocks.
"""
...
def test_alembic_upgrade_head_adds_chat_sessions_tags(tmp_path, monkeypatch):
