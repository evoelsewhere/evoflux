...
class TestExceptionHierarchy:
    ...
    def test_all_inherit_from_EVOFLUX_error(self):
        for exc_cls in (
            ProviderError,
            ProviderRateLimitError,
            ProviderConnectionError,
            ...
            assert issubclass(exc_cls, EvoFluxError), f"{exc_cls.__name__}"
    ...
    def test_sandbox_also_permission_error(self):
        """SandboxError inherits from both EvoFluxError and PermissionError."""
    ...
    def test_can_raise_and_catch(self):
        with __import__("pytest").raises(EvoFluxError):
