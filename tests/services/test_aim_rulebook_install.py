...
def test_installs_pack_workflows_and_skills_gap_fill(config_dirs, monkeypatch, tmp_path):
    ...
    assert installed == {"workflows": ["demo-flow.yaml"], "skills": ["demo-skill"]}
    ...
    assert installed2 == {"workflows": [], "skills": []}
...
def test_unknown_pack_installs_nothing(config_dirs):
    ...
    assert install_rulebook_content("not-a-pack") == {"workflows": [], "skills": []}
