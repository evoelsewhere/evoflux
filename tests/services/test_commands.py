...
def test_discover_finds_command_in_each_root(roots):
    ...
    assert result["a"].source == "project-EvoFlux"
    assert result["b"].source == "project-opencode"
    assert result["c"].source == "global-EvoFlux"
    assert result["d"].source == "global-opencode"


def test_precedence_project_EVOFLUX_wins_over_global(roots):
    cwd, proj_oad, proj_oc, global_oad, global_oc = roots
    _write(
        proj_oad / "commit.md",
    ...
    assert result["commit"].source == "project-EvoFlux"
