...
StructuralParsers for *source* workspaces only → repository-local parser
registry → code index refresh → ``code_context`` search finds legacy
symbols.
"""
...
async def test_reindex_indexes_cobol_estate_and_search_finds_paragraph(
    ...
    async with async_session_factory() as db:
        ...
        assert project.settings["aim"]["roles"]["source"]
        await refresh_code_index_bridge(db)
        target_path = tmp_path / "estate-target"
        await db.commit()

    source_index = await repository_indexes.get(source)
    try:
        stats = await source_index.update(full=True)
        assert not stats.errors
        assert stats.files == 2  # the .cbl and the .jcl
        assert stats.symbols > 0

        scope = (RepositoryScope(root=source, label="source"),)
        ...
        assert any("CALC-PARA" in hit.content for hit in result.hits)

        paragraph_result = await query_code_context(
            scopes=scope,
            action="definition",
            query="CALC-PARA",
            refresh=False,
        ...
        assert any(
            match.name == "CALC-PARA" and match.kind == "paragraph"
            for match in paragraph_result.matches
        ), paragraph_result.matches

        job_result = await query_code_context(
            scopes=scope,
            action="definition",
            query="STEP010",
            refresh=False,
        ...
        assert any(
            match.name == "STEP010" and match.kind == "step"
            for match in job_result.matches
        ), job_result.matches

        # The same .cbl content in the TARGET workspace is ignored —
        # extractors are scoped to source-role workspaces, so no parser claims
        # .cbl there.
        (target_path / "leftover.cbl").write_text(COBOL_SAMPLE, encoding="utf-8")
        target_index = await repository_indexes.get(target_path)
        try:
            target_stats = await target_index.update(full=True)
            assert target_stats.files == 0
        finally:
            await repository_indexes.purge(target_path)
    finally:
        await repository_indexes.purge(source)
