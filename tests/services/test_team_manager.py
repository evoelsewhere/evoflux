...
def test_existing_aim_roster_migrates_all_legacy_skill_names(tmp_path):
    agents_dir = tmp_path / "aim"
    agents_dir.mkdir()
    specialist = agents_dir / "custom-specialist.md"
...
""",
    ...
    assert parse_agent_md(specialist).skills == [
        "work-decision",
        "work-planning",
        "work-writing",
...
def test_existing_aim_roster_skill_migration_is_idempotent(tmp_path):
    agents_dir = tmp_path / "aim"
    agents_dir.mkdir()
    specialist = agents_dir / "custom-specialist.md"
    specialist.write_text(
...
role: member
model: custom:model
skills: [decision-analysis, custom-skill]
---
Custom body.
""",
        encoding="utf-8",
    ...
    team_manager._ensure_aim_agents_installed(agents_dir)
    first_migration = specialist.read_text(encoding="utf-8")
    team_manager._ensure_aim_agents_installed(agents_dir)

    assert specialist.read_text(encoding="utf-8") == first_migration


def test_existing_aim_roster_migration_preserves_custom_skills_and_content(tmp_path):
    agents_dir = tmp_path / "aim"
    agents_dir.mkdir()
    specialist = agents_dir / "custom-specialist.md"
    specialist.write_text(
...
role: member
model: custom:model
custom-setting: keep-me # preserve this custom frontmatter verbatim
skills:
  - custom-skill
  - decision-analysis
  - coding-testing
...
---
The body may still discuss decision-analysis and must stay untouched.
""",
        encoding="utf-8",
    )
    ...
    content = specialist.read_text(encoding="utf-8")
    assert (
        "custom-setting: keep-me # preserve this custom frontmatter verbatim" in content
    )
    assert (
        "The body may still discuss decision-analysis and must stay untouched."
        in content
