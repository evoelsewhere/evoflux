<div align="center">
  ...
  **[Watch the real EASD run](documents/images/easd-ui-end-to-end.mp4)**
</div>
...
> [!NOTE]
> Tagged updater archives are minisign-signed. Native package signing depends
> on the platform credentials configured for the release workflow; the
> operating system may show a trust prompt when a native package is unsigned.
...
[![Play the real EASD end-to-end run](documents/images/easd-ui-end-to-end.jpg)](documents/images/easd-ui-end-to-end.mp4)
...
[![Play the real EASD run inspection](documents/images/easd-ui-run-inspection.jpg)](documents/images/easd-ui-run-inspection.mp4)
...
Thirty-nine built-in skills cover mode-scoped Coding workflows, Office and PDF document production, research and analysis, design and product workflows, EvoFlux configuration and installers, portable plugin development, and provider-neutral PR lifecycle operations. Work and Coding each expose one implicit router; broad specialists are explicit-only so they do not compete on every request. Custom skills can be created, edited, diagnosed, and filtered as Work, Coding, or Both in Settings. A bounded 2%/8K metadata catalog is always available for model-driven selection, while `SKILL.md` bodies and bundle resources load only after exact activation. EvoFlux is also an MCP client for stdio and Streamable HTTP servers; connected tools inherit the same permission rules as native tools.
...
documents/  Architecture, feature contracts, records, media, and EASD run data
```
