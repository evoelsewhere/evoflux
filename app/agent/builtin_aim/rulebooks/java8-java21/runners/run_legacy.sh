#!/usr/bin/env bash
# Runs a migration unit's legacy (Java 8/11) implementation and writes its
# canonical-ready output for aim_compare. Customize per project — this is a
# starting template, not a working script for any specific estate.
#
...
echo "run_legacy stub — received: UNIT='$UNIT' CASE_SET='$CASE_SET' OUT_DIR='$OUT_DIR'" >&2
echo "Contract: \$1=UNIT  \$2=CASE_SET (smoke|full)  \$3=OUT_DIR (absolute output dir)" >&2
echo "TODO: build and run the legacy (Java 8/11) jar/service for unit '$UNIT', case set '$CASE_SET'" >&2
