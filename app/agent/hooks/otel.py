...
- ``EvoFlux.tool.execution.duration``   — histogram (seconds) per tool call
- ``EvoFlux.agent.runs.total``          — counter per completed agent run
...
Usage::
    ...
    hook = OpenTelemetryHook(agent_name="EvoFlux", model_id="openai:gpt-4o")
...
class OpenTelemetryHook(BaseAgentHook):
    ...
    def __init__(
        ...
        self._tool_duration = self._meter.create_histogram(
            name="evoflux.tool.execution.duration",
            description="Tool execution duration",
            unit="s",
        )
        self._runs_counter = self._meter.create_counter(
            name="evoflux.agent.runs.total",
