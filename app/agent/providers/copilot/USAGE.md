...
In EvoFlux's agent loop, each **LLM call** counts as a prompt — not just the user's initial message. A single user message that triggers N tool calls = ~(N+1) LLM calls.
...
## Recommended models for EvoFlux
