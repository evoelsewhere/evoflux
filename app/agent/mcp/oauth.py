...
def has_cached_oauth_tokens(name: str) -> bool:
    path = Path(settings.EVOFLUX_CACHE_DIR) / "mcp-oauth" / f"{name}.json"
    if not path.is_file():
        return False
    try:
...
class LoopbackCallback:
    ...
    async def wait(self) -> AuthorizationCodeResult:
        ...
        try:
            ...
            # MCP 2.x models the callback payload explicitly so the OAuth
            # provider can validate the authorization response consistently.
...
class FileTokenStorage:
    def __init__(self, name: str, cfg: HttpServerConfig) -> None:
        self._path = Path(settings.EVOFLUX_CACHE_DIR) / "mcp-oauth" / f"{name}.json"
        self._cfg = cfg
