...
Both resolve the current AIM project through the workspace/project registry
(``CodingWorkspace`` -> ``CodingProject``),
...
async def _resolve_project_and_kb_root(db) -> tuple[UUID | None, Path]:
    sandbox = get_sandbox()
    workspace_path = str(Path(sandbox.workspace_root).expanduser().resolve())
    workspace = (
        await db.exec(
            select(CodingWorkspace).where(CodingWorkspace.path == workspace_path)
        )
    ).first()
