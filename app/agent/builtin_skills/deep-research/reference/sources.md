...
If the `web_search` tool is missing or errors, use DuckDuckGo HTML via `web_fetch` — verified to return full SERP results:
...
- For Chinese-language topics, also try Bing: ``web_fetch` https://www.bing.com/search?q=<query>&setlang=zh` (works without key, quality varies).
...
Full text of an arXiv paper: `web_fetch` `https://arxiv.org/abs/<id>` for abstract, or `https://ar5iv.org/abs/<id>` for HTML full text.
...
- Reddit: `web_fetch` `https://www.reddit.com/search/?q=<q>` or append `.json` to any thread URL
