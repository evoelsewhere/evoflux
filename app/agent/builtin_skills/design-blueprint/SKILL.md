...
description: "Use this skill to produce a design specification, a structural layout, and a decision trace before any visual artifact is built, for decks, landing pages, dashboards, posters, charts, infographics, or interface components. Apply it when a design needs a point of view before pixels, or when critiquing an existing design against a principled spec. Do not use it when an accepted specification already exists and only implementation remains."
---
