...
description: "Use this skill to build or debug a pipeline that renders an HTML page into a video file through headless-browser capture and ffmpeg. Apply it to deterministic frame stepping, real-time recording, multi-scene concatenation, and symptoms such as fallback-font flashing, frozen opening seconds, truncated animations, wildly wrong output duration, blurry export, or stuttering playback. Do not use it to edit existing footage, to author the page itself, or to record a live session."
---
