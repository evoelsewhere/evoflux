...
- Also `web_fetch` survey papers' related-work sections and awesome-lists — they surface what keyword search doesn't rank.
...
- For parallel extraction across 10–25 papers, delegate members with `team_delegate` in batches of 3–5, each with the fetched text file paths (not summaries) and the field schema from `question.md`.
