...
license: "Adapted from iOfficeAI/OfficeCLI (Apache License 2.0) — https://github.com/iOfficeAI/OfficeCLI"
---

## When to Use

- Creating, reading, editing, or fixing any .xlsx/.xlsm/.csv/.tsv file
- Financial models, trackers, dashboards, pivot tables, charts
- Cleaning or restructuring messy tabular data into a proper spreadsheet

## When NOT to Use

- The deliverable is a Word document, HTML report, standalone script, or database pipeline — even if tabular data is involved
- Google Sheets API integration (not a local file)

# OfficeCLI XLSX Skill

EvoFlux bundles the `officecli` binary with the desktop app (see `scripts/build_sidecar.py`) and puts it on `PATH` automatically at startup — you normally don't need to install anything. The setup step below is only a fallback for dev/CI environments running outside the bundled sidecar.
...
**Native `import` command (preferred for CSV/TSV).** Fastest path; loads a CSV into a sheet in one call. `--header` sets AutoFilter + freeze pane on row 1. Widths and `numFmt` still need a follow-up pass.
