---
name: compose-next
description: "Use this skill only when the user explicitly asks for the Compose Next delivery workflow, by name or in clear natural language. It carries one feature, fix, or refactor through requirement resolution, workspace setup, specification, implementation, verification, independent review, and hand-off in a single contract. Do not infer it from task size, do not use it for one-shot edits or questions, and prefer the repository's own EASD workflow when project instructions require it."
---
...
This repository runs EASD, the Evo Agent Specification-Driven method, and its
`easd-*` skills own specification, planning, implementation, review, and
verification here. When project instructions require EASD, follow EASD and use
this skill only for the parts it does not cover. Prefer the `worktree_start`
and `worktree_finish` tools over raw `git worktree` commands wherever they
apply.
...
- When choices cannot be enumerated, still call `ask_user` with `options: []` for free-text.
...
If the `ask_user` tool is unavailable or returns `[Never-Ask]`, resolve **this one decision** yourself and continue:
...
Never-Ask applies to the current decision only. At every later decision point, call the `ask_user` tool again — Never-Ask does not disable future questions or pause the workflow.
...
After implementation is verified and before finalizing the feature document, delegate one fresh reviewer with `team_delegate` to review the complete change.
...
If the user asks to finish but the path is unclear, use the `ask_user` tool to settle:
