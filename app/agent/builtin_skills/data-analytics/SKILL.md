---
name: data-analytics
description: "Use this skill for quantitative product or business analysis delivered through its bundled workflows: data-quality checks, metric diagnostics, KPI design and reporting, dashboards, analytical reports, notebooks, market sizing, and evidence-backed recommendations. Do not use it for spreadsheet formatting alone, for external fact research, or for building a data pipeline whose real deliverable is production code."
---

# Data Analytics

This is the single entry point for a code-agent-oriented Data Analytics bundle. The original plugin workflows are under `references/workflows/`; React, Vite, MCP App widgets, connector manifests, and frontend build artifacts are intentionally excluded.

## Route the request

1. Read `references/workflows/index/SKILL.md` completely. Treat it as the authoritative plugin-level eligibility and analytical routing policy, except where its UI/runtime delivery rules conflict with the code-agent overrides below.
...
Focused workflows live in `references/workflows/`, including data-quality analysis, business-context gathering, metric diagnostics, product/business analysis, KPI design and reporting, visualization, dashboards, reports, notebooks, market sizing, validation, and semantic-layer creation.
