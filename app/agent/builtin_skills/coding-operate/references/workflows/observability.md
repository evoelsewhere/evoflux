...
description: Use this workflow to add or redesign logs, metrics, traces, or alerts for a code path — deciding what to instrument and why, structured-logging shape, correlation IDs, RED/USE signal selection, or alert thresholds. It requires naming the on-call question the telemetry answers before adding it; do not use it for functional correctness work with no diagnostic-visibility gap, or for measuring a single already-identified performance bottleneck.
---
