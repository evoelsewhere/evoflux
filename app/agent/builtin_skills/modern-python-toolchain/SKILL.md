...
description: "Use this skill to set up or repair a Python project's toolchain: environment and dependency management with uv, linting and formatting with Ruff, type checking with Pyright, and the checks to run after editing Python. Apply it when a project needs initializing, its tooling is missing or inconsistent, or a dependency or Python version must change. Do not use it for application logic, test design, or a repository whose established toolchain is not part of the task."
---
