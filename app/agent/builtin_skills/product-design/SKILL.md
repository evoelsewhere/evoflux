---
name: product-design
description: "Use this skill for product and user-experience design carried out through its bundled workflows: gathering product and user context, design research, evidence-grounded audits, ideation, turning a reference URL or image into a prototype, design QA, and hand-off. Apply it when the deliverable is a designed experience or a principled design critique. Do not use it for production frontend implementation, static artwork, or document and slide formatting."
---

# Product Design

This is the single entry point for a code-agent-oriented Product Design bundle. The original workflow instructions are under `references/workflows/`, with shared guidance under `references/`. The bundled React/Vite prototype template and package metadata are intentionally excluded.

## Route the request

1. Read `references/workflows/index/SKILL.md` completely. Treat it as the authoritative plugin-level scope, routing, sequencing, and browser-choice policy.
...
Focused workflows live in `references/workflows/`: `user-context`, `get-context`, `research`, `audit`, `ideate`, `url-to-code`, `image-to-code`, `design-qa`, and `share`.
