...
Maps the :class:`~app.core.errors.EvoFluxError` hierarchy to HTTP status codes.
...
async def _EVOFLUX_fallback(request: Request, exc: EvoFluxError) -> JSONResponse:
    """Catch-all for any EvoFluxError subclass not handled by a more specific entry."""
...
EXCEPTION_HANDLERS: dict[int | type[Exception], _ExceptionHandler] = {
    ...
    EvoFluxError: _EVOFLUX_fallback,
}
