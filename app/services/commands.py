...
name collision, later sources are silently ignored:
    ...
    2. ``{workspace}/.opencode/commands/``    (project, opencode reuse;
                                               coding mode only)
    3. ``{EVOFLUX_CONFIG_DIR}/commands/``     (global, EvoFlux)
...
class Command:
    ...
    source: str  # one of: project-EvoFlux / project-opencode / global-EvoFlux / global-opencode
...
def _candidate_roots(workspace: Path | None = None) -> list[tuple[Path, str]]:
    ...
    roots.extend(
        [
            (config / "commands", "global-EvoFlux"),
...
# ── Built-in commands ───────────────────────────────────────────────────────
#
# Built-ins are prompt templates owned by EvoFlux itself rather than the
