...
class AimIndexWatcher:
    ...
    def is_relevant(path: str) -> bool:
        ...
        return normalized == "aim.yaml" or normalized.startswith(
            ("modules/", "rulebook/", "runs/", "state/")
        )

    async def start(self) -> None:
        async with self._db_factory() as db:
            from app.services.aim.extractors import refresh_code_index_bridge
    ...
    async def unwatch_project(self, project_id: UUID) -> None:
        """Drop callbacks and pending rebuilds owned by one deleted project."""

        async with self._lock:
            paths = [
                path
                for path, watched_project_id in self._projects.items()
                if watched_project_id == project_id
            ]
            for path in paths:
                self._projects.pop(path, None)
