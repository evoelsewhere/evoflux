...
def upgrade() -> None:
    ...
    indexes = {
        index["name"]
        for index in sa.inspect(op.get_bind()).get_indexes("remote_connections")
    }
    if "ix_remote_connections_enabled" not in indexes:
        op.create_index(
            "ix_remote_connections_enabled", "remote_connections", ["enabled"]
        )

    if "remote_pairings" not in set(sa.inspect(op.get_bind()).get_table_names()):
        op.create_table(
            "remote_pairings",
            sa.Column("id", sa.Uuid(), primary_key=True),
            sa.Column(
            ...
            sa.Column("label", sa.String(length=120), nullable=False),
            sa.Column(
                "display", sa.String(length=120), nullable=False, server_default=""
            ),
            sa.Column(
                "active_session_id",
                sa.Uuid(),
    ...
    indexes = {
        index["name"]
        for index in sa.inspect(op.get_bind()).get_indexes("remote_pairings")
    }
    if "ix_remote_pairings_connection" not in indexes:
        op.create_index(
