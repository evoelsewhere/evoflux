...
- Top-level config files: ``mcp.json`` plus generated ``settings.yaml``.
  These are the defaults the user can edit later;
...
class SeedResult:
    ...
    configs_written: list[str]  # top-level mcp.json/settings.yaml/etc.
...
def write_env_credentials(
    ...
    *,
    ...
    ----------
    env_file
        Absolute path to ``~/.config/EvoFlux/.env`` (or the test
        equivalent).
    credentials
...
def install_seed(
    ...
    *,
    ...
    ----------
    config_dir
        Target ``{EVOFLUX_CONFIG_DIR}`` — ``agents/`` and ``skills/`` are
