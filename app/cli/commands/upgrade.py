...
def cmd_upgrade(args: argparse.Namespace) -> None:
    """Upgrade EvoFlux to the latest version."""
    was_running = bool(_find_pids())
    if was_running:
        print(f"  {_bold('Stopping EvoFlux')} before upgrade ...")
        cmd_stop(args)

    manager, command = _upgrade_command()
    print(f"  {_bold('Upgrading EvoFlux')} via {_cyan(manager)} ...")
    upgrade_code = 0
    ...
    if was_running:
        ...
        if upgrade_code == 0:
            print(f"  {_bold('Restarting EvoFlux')} ...")
        else:
            print(f"  {_bold('Restarting EvoFlux')} after failed upgrade ...")
