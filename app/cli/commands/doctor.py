...
def cmd_doctor(_args: argparse.Namespace) -> None:
    ...
    if alembic_ini.is_file():
        ...
        _fail(f"Alembic config missing: {alembic_ini}  (reinstall EvoFlux)")
...
def _read_lead_provider(agents_dir: Path) -> str | None:
    ...
    Looks for ``EvoFlux.md`` first (the seed lead) and falls back to the
