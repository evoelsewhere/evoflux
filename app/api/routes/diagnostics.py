...
async def diagnostics(tail: int = 200) -> dict[str, Any]:
    ...
    return {
        ...
        "dirs": {
            "data": _dir_info(settings.EVOFLUX_DATA_DIR),
            "config": _dir_info(settings.EVOFLUX_CONFIG_DIR),
            "state": _dir_info(settings.EVOFLUX_STATE_DIR),
            "cache": _dir_info(settings.EVOFLUX_CACHE_DIR),
            "wiki": _dir_info(settings.EVOFLUX_WIKI_DIR),
            "workspace": _dir_info(settings.EVOFLUX_WORKSPACE_DIR),
            "agents": _dir_info(settings.AGENTS_DIR),
            "skills": _dir_info(settings.SKILLS_DIR),
        },
        "providers": _provider_status(),
        "env": _safe_env_keys(("EVOFLUX_", "APP_", "PYTHON")),
        "team": {
