...
**AIM (AI Innovation Modernization)** là **mode thứ ba của EvoFlux** — đứng cạnh Work và Coding — chịu trách nhiệm riêng cho dòng dự án migration legacy: *hiểu source cũ → convert sang source mới → chứng minh tương đương chức năng (test compare)*. AIM không đụng vào hai mode hiện có; nó tái sử dụng toàn bộ core harness (lead-and-mailbox, code index, skills, permissions, streaming UI) với workspace model, roster và tool tier riêng.
...
|---|---|
| Code comprehension deterministic | Repository-local code index với parser tree-sitter ~20-25 ngôn ngữ, FTS5, symbol relations và truy vấn đa repo qua `code_context` (`app/services/code_index/`) |
...
| **Khái niệm mode đã là kiến trúc lõi** | Work/Coding chia sẻ cùng core lead-and-mailbox, streaming UI, permission model — thêm mode thứ 3 là mở rộng theo đúng thớ thiết kế (thậm chí đã có tiền lệ đổi mode: `normalize_mode` từng map `normal`→`work`, `app/models/chat.py:12-20`) |
...
| Tooling thêm | — | Git, code index, file tree | **Code index trên cả legacy (structural parser) + `aim_units`/`aim_compare` + golden store + rulebook + dashboard factory** |
...
1. **Base source** — repo(s) legacy. **Nhiều module / nhiều repository là trường hợp mặc định, không phải ngoại lệ**: `roles.source` là danh sách; `code_context` truy vấn trên mọi source repo được cấp quyền và resolve quan hệ CALL/COPY/import; unit đặt tên namespace `<module>/<unit>`. Mount **read-only**: đây là bất biến của mode, enforce ở tầng sandbox/permission (xem dưới), không phụ thuộc prompt.
...
- **Tool tier `"aim"`**: `aim_units`/`aim_compare` đăng ký `tiers=("aim",)`; các tool nền cần cho modernization (fs/shell/python/git/`code_context`/LSP/worktree) thêm `"aim"` vào tiers hiện có. Registry đã hỗ trợ sẵn cơ chế này.
...
Watcher file theo project refresh dữ liệu derive khi KB đổi: **`git pull` xong là board cập nhật**.
...
**Comprehension pipeline** (kiểu CodeConcise, bottom-up): xếp unit leaves-first theo symbol relations calls/imports/contains trong repository-local index → mỗi unit một turn agent đọc source + neighborhood từ `code_context` + **docs của các callee đã viết trước đó** → ghi `modules/<module>/<unit>.md` + trích candidate `business-rules/` → human gate xác nhận rule theo batch. Phía target: một pass nhẹ hơn trích `target-conventions.md` từ target base (đọc skeleton + build files + sample code). Trước khi có Workflows engine, pipeline chạy bằng slash command `/aim-understand-unit`.

**Nhiều source repo & nhiều người cùng xây KB** (mô hình vận hành):

- **Estate nhiều module/repo là mặc định**: `roles.source` trong `aim.yaml` là danh sách; `code_context` hợp nhất kết quả và quan hệ CALL/COPY từ mọi source repo được cấp quyền; wave thường trùng ranh giới module; `interfaces/` ghi **hợp đồng giữa các module** — thứ quan trọng nhất khi hai nhóm convert hai module có nói chuyện với nhau.
...
- **Tier 1 — tree-sitter thật** khi grammar đủ tốt: subclass parser nền và đăng ký (pattern có sẵn `app/services/code_index/parsers/base.py`). Java/C#/Pascal-Delphi đã có sẵn → rulebook Java 8→21 **không cần việc parser nào**.
- **Tier 2 — structural fallback parser (mặc định cho COBOL/JCL/PL-I/RPG/VB6)**: xem 3.9.
- **Tier 3 — LLM enrichment vào KB** cho phần extractor không thấy: chunk theo đơn vị structural của Tier 2, viết prose vào `modules/`. Retrieval giữ lexical FTS + symbol relations + KB markdown — **không vector RAG naive** (backend vector tuỳ chọn `memory_vector.py` bật sau được mà không đổi thiết kế).

### 3.7 Project rulebook — tri thức thuộc KB của engagement

Rulebook là một **artifact của project**, chỉ tồn tại tại `<kb-repo>/rulebook/` và được pin identity/version bởi `aim.yaml`. EvoFlux không ship catalog stack-specific, không chọn pack khi create, không fallback, và không cài nội dung rulebook vào discovery root toàn cục. Project mới nhận sample an toàn từ `seed/aim-kb-template/rulebook/`; toàn bộ capability ban đầu là `template` cho đến khi engagement adapt và kiểm chứng.
...
agents/*.example.md      # guidance tham khảo; không merge vào roster toàn cục
skills/*/SKILL.example.md # tri thức tham khảo; không auto-discover/install
commands/*.md            # tài liệu operator của project; không auto-install
workflows/*.example.yaml # workflow tham khảo; generic lifecycle vẫn là builtin
...
runners/*.sh|*.py        # adapter được manifest khai báo và chạy trực tiếp tại chỗ
...
**Ngữ nghĩa runtime**: `app/services/aim/rulebook.py` chỉ resolve và validate `<kb>/rulebook/rulebook.yaml`; identity phải khớp `aim.yaml`, mọi declared path phải nằm trong thư mục rulebook. `mappings/canonicalizers/extractors/runners` được tham chiếu tại chỗ; example không được load nếu manifest chưa khai báo. `agents/skills/workflows/commands` trong rulebook chỉ là guidance để team review và chuyển sang cấu hình project bằng cơ chế bình thường khi thực sự cần.

Tri thức học được trong engagement được commit cùng KB, review qua PR, và đi cùng audit trail của project. Việc tái sử dụng sang project khác là một quyết định copy/adapt có chủ đích, không phải EvoFlux âm thầm phân phối hay nâng cấp rulebook dùng chung.
...
**(d) Repair loop**: v5 Phase 1 cấm cycle trong DAG (validate tại save), nên vòng lặp sửa nằm **trong turn của converter agent**: node `repair` là agent node có `aim_compare` trong tool của blueprint, tự lặp fix→compare trong turn (kỷ luật `coding-testing`) đến khi pass hoặc hết budget; node compare **cuối** của workflow là verdict chính thức. Trung thực với ngữ nghĩa v5, không cần sửa engine.

**(e) Dashboard data**: `aim_units` + `aim_runs` phía sau `GET /api/team/projects/{id}/aim/summary` (+ `/units`, `/runs/{id}`), đặt trong nhóm route project hiện có tại `app/api/routes/team/projects.py`.

### 3.9 Structural fallback parser — đưa legacy vào code index

Điểm mấu chốt đã xác minh: `LanguageParser` Protocol chỉ yêu cầu `name`, `extensions`, `parse(file_path, source) -> ParseResult` — **không bắt buộc tree-sitter**, và header của `app/services/code_index/parsers/registry.py` tuyên bố rõ đây là extension point ("Nothing else in the pipeline needs to change").
...
- **Thay đổi engine duy nhất**: `build_registry()` nhận `extra_parsers`; AIM đăng ký provider theo source root qua `refresh_code_index_bridge()`, còn `RepositoryIndex` tự lấy `registry_for_root(root)` và giữ nguyên pipeline index/query.
- **Kết quả**: code legacy vào repository-local chunks/symbols/relations + FTS5 → tool thống nhất `code_context` hỗ trợ search, definition, callers/callees, references và impact cho COBOL/JCL — nền "deterministic parsing + graph TRƯỚC, LLM SAU" thành hiện thực cho legacy.
...
|---|---|---|---|
| `aim-lead` | **lead** | Điều phối toàn pipeline | Hiểu state machine `aim_units`; quyết định phase/wave tiếp theo; gọi đúng workflow; tổng hợp báo cáo. Tool: `aim_units`, `code_context`, delegate |
| `aim-appraiser` | member | 0 — Assess | `code_context`, `aim_units`; wave planning, complexity scoring |
| `aim-archaeologist` | member | 1 — Understand | `code_context`, fs write vào KB; skill `aim-legacy-comprehension`, `coding-investigation` |
| `aim-target-architect` | member | 1b+2 — Target conventions & Design | `coding-migration`, `coding-investigation`, `work-decision` + `mappings/` của rulebook + `target-conventions.md` |
| `aim-converter` | member | 3+4 — Convert/Repair | worktree_start/finish, edit/patch, shell, `aim_compare`; `coding-implementation`, `coding-testing`, `coding-debugging`, `coding-migration` |
...
nodes:
  - id: overview
    ...
    args:
      action: search
      query: "top-level modules, entrypoints, and dependency structure"
      limit: 20
  - id: inventory
    kind: agent
    ...
    prompt: |
      ...
      Dùng code_context cho dependencies; ghi units, complexity và
      wave plan (wave size mục tiêu {{inputs.wave_size}}) qua tool aim_units,
      và viết inventory/units.md trong KB repo.
      Tổng quan code index: {{nodes.overview.output.text | truncate:4000}}
  - id: wave_gate
    kind: gate
...
> **Rev 2026-07-16 (sau khi backend core AIM-0→3 xong, user chốt IA):** điều hướng của mode là **sidebar → project → tính năng con → main content** — switch mode AIM thì sidebar list các AIM project; click project expand các tính năng con dạng dropdown (Overview/Board, Knowledge Base, Rulebook, Pipelines, Runs & Reports); main content render tính năng đang chọn và làm việc tại đó. Board không còn là "một màn duy nhất" mà là tính năng **Overview** mặc định của project. **User siết thêm: mode này KHÔNG hướng đến chat** — trigger pipeline là UI thuần (form + nút Run, FE tự resolve session per-run và auto-send command dưới nắp, cùng execution substrate); chat chỉ còn là **Discussion tuỳ chọn trên một run ĐÃ CHẠY XONG** (đọc transcript + hỏi thêm), không có nút Chat ở header, không composer thường trực. Session vẫn per-run. Chi tiết + wireframes + thứ tự build FE-1→FE-4: [`documents/plans/aim-mode-shell-ux-spec.md`](../plans/aim-mode-shell-ux-spec.md) **v2.1** (chờ duyệt theo R7). Các nguyên tắc flow-first bên dưới giữ nguyên giá trị — chúng mô tả *nội dung* các surface; IA mới chỉ thay *cách điều hướng đến* chúng, và mọi chỗ bên dưới nói "chat drawer để trigger/bổ sung ngữ cảnh" được thay bằng mô hình Discussion hậu-run này.

**Nguyên tắc: màn hình chính của AIM là dòng chảy migration, không phải chat.** Persona vận hành AIM (delivery lead / PM / factory operator) đặt câu hỏi dashboard — "cái gì đang ở đâu, cái gì blocked, cái gì chờ tôi duyệt, equivalence bao nhiêu %" — và tương tác theo **sự kiện** (gate cần duyệt, run fail, unit được certify), không theo hội thoại. Work/Coding là chat-first vì công việc là exploratory; AIM là factory nên **flow-first**. Chat vẫn tồn tại nhưng là **drawer tuỳ chọn**: bổ sung ngữ cảnh cho agent, hỏi đáp ad-hoc ("vì sao unit này fail hoài?"), trigger thủ công, debug. EvoFlux đã có tiền lệ surface không-phải-chat làm màn hình chính (`/telemetry`, `/scheduler`, Monitor view) — AIM đi cùng thớ đó.
...
|---|---|---|
| **AIM-0 — Project knowledge foundation** ✅ **ĐÃ IMPLEMENT** | Roster AIM và method skills là builtin stack-agnostic; generic workflow nằm tại `app/agent/builtin_aim/workflows/`; KB scaffold tại `seed/aim-kb-template/` chứa active sample `rulebook/` với mọi capability ở mức `template`. EvoFlux không ship stack-pair rulebook. | Project mới luôn có rulebook local an toàn, Git-track được; không có catalog/package fallback hoặc global content install. |
| **AIM-1 — State + compare spine** ✅ **ĐÃ IMPLEMENT** (2026-07-16, mode-agnostic) | `app/models/aim.py` (AimUnit/AimRun/AimLink) + cột `kind` trên `coding_projects`, migration `00000020` (verified upgrade/downgrade round-trip); `app/services/aim/{models,kb_store,reindex,canonicalize,compare}.py` — KB frontmatter là system of record, bảng là index rebuildable qua `upsert_unit`/`reindex_project`; tool `aim_units` (get/list/set_phase/set_project_phase/record_run/add_link, `tiers=("aim",)`) + `aim_compare` (canonicalize→JSON-aware diff với tolerance + sort-before-diff→report.json/md, tự ghi `AimRun`), cả hai đăng ký trong `app/agent/loader.py`; API `GET /team/projects/{id}/aim/{summary,units,runs/{run_id}}` tại `app/api/routes/team/projects.py` | Golden case so sánh deterministic được (test compare pass/fail + tolerance + mask đều pass); agent tự hoàn thành vòng fix→compare chỉ bằng tools; **xác nhận bằng test: `tier_tools(mode="work"\|"coding")` KHÔNG thấy `aim_units`/`aim_compare` — zero impact 2 mode hiện có**; watcher tự động reindex sau `git pull` (đồng bộ nhiều máy) **chưa làm** — hiện tại reindex chạy qua tool call trực tiếp, không qua file-watcher |
| **AIM-2 — Mode shell** — ✅ **HOÀN CHỈNH backend + frontend** (2026-07-16; backend e13f4ef; FE theo spec v2.2 đã duyệt: FE-1+FE-2 = 2258124, FE-3 = b19bf84, FE-4 = a50a1bd — sidebar→project→feature IA, wizard 1-folder-pick auto-detect, Overview kanban, Pipelines trigger UI thuần + bảng run, KB browser read-only + Reindex, Runs & Reports, Discussion hậu-run là chat surface duy nhất, Rulebook viewer + endpoint GET .../aim/rulebook; verified live qua browser trên project core-batch thật) | Backend đã xong: `mode="aim"` + validation (schemas/chat.py, routes/team/chat.py); tool tier `aim`; roster `seed/agents/aim/` load qua `_resolve_aim_agents_dir` + self-heal `_ensure_aim_agents_installed` (model lấy theo lead work đã cấu hình); **read-only base source** — `SandboxConfig.read_only_paths` mới (chặn write, vẫn cho read; `validate_path(is_write=True)` ở write/rm/edit/patch + chặn redirect `>`/`>>` trong `check_command`, ghi rõ giới hạn không bắt `sed -i`); API `POST /team/projects/aim` (create: scaffold KB template + sinh `aim.yaml` theo repo identity) / `POST /team/projects/aim/preview` / `POST /team/projects/aim/join` (map identity→local path, không hỏi lại config) tại `app/services/aim/project_setup.py`; `GET /team/projects?kind=` filter — Coding/Work không thấy AIM project. UX spec đã duyệt tại [`documents/plans/aim-mode-shell-ux-spec.md`](../plans/aim-mode-shell-ux-spec.md). **Phần frontend còn lại (route `/aim/$focusId` + AIM Board skeleton + chat drawer + `AimSetupWizard`) hoãn** — spec giữ nguyên giá trị khi làm lại; bản nháp FE đầu tiên đã bị discard chủ động | Backend DoD đã đạt: session mode aim chạy với roster AIM (verified qua browser + curl: aim-lead trả lời trong session aim, model foundry:claude-sonnet-5); tạo project qua API tạo đủ aim.yaml/workspace mapping/settings["aim"]; join bằng KB có sẵn chỉ hỏi mapping local (7 test project_setup); mọi tool ghi bị chặn trên base source (15 test sandbox/filesystem read-only); Coding/Work không thấy AIM project và ngược lại (test kind filter). FE DoD (mở `/aim/<project>` thấy Board, wizard) chuyển sang milestone FE sau |
| **AIM-3 — Legacy graph** ✅ **ĐÃ IMPLEMENT** | `StructuralParser` nhận extractor được project khai báo tường minh trong `<kb>/rulebook/rulebook.yaml`; chỉ source-role workspace nhận parser. Sample extractor dùng hậu tố `.example.yaml` và không chạy cho tới khi được rename, review, rồi declare. | Integration test tạo extractor COBOL/JCL ngay trong KB, index estate mẫu và tìm được paragraph/step; target, KB và non-AIM workspace không nhận extractor. |
| **AIM-4 — Pipelines** ✅ **ĐÃ IMPLEMENT** | Generic AIM workflows thuộc builtin discovery root. Readiness đọc capability maturity từ local rulebook; runner/canonicalizer/extractor được resolve tại chỗ và bị giới hạn trong `<kb>/rulebook/`. Không có workflow, skill hoặc agent nào được cài từ rulebook. | Pipeline chỉ chạy khi capability local là `ready`; sample mới giữ tất cả ở `template`, vì vậy project không thể vô tình chạy policy chưa adapt. |
...
7. **Work/Coding có được mở AIM project không**: mặc định không (filter theo `kind`), nhưng có nên cho Coding mode "attach read-only" vào target repo của AIM project khi cần sửa ngoài luồng? Cần quy tắc rõ để không phá traceability.
...
| Parser extension point (Protocol không cần tree-sitter) | `app/services/code_index/parsers/registry.py`; `app/services/code_index/parsers/base.py`; bridge AIM tại `app/services/aim/extractors.py` |
