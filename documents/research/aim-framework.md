...
| **AIM-3 — Legacy graph** ✅ **ĐÃ IMPLEMENT** (2026-07-16, song song AIM-2) | `app/services/code_graph/parsers/structural.py` — StructuralParser config-driven (node_rules scope file/block flat, edge_rules named-group, keyword_denylist, guardrails như base.py; load-and-skip config hỏng); hook `build_registry(extra_parsers=...)` (extras thắng khi trùng extension); extractor configs: pack skeleton `cobol-java21` (rulebook.yaml `parser_strategy: structural` + `extractors:` list) với `cobol-structural.yaml` (program/division/section/paragraph nodes; PERFORM/CALL/COPY/EXEC CICS LINK/EXEC SQL INCLUDE edges) + `jcl-structural.yaml` (job/step nodes; PGM=/PROC/DSN edges); `vb6-structural.yaml` chuẩn hoá executable (denylist keyword, target_group); wiring `app/services/aim/extractors.py`: `structural_parsers_for_workspace` chỉ trả parser cho workspace **role source** của AIM project (rulebook từ `settings["aim"]`), gọi lazily trong `reindex_workspace`; skill `aim-legacy-comprehension` thêm mục "Deriving the bottom-up order" (leaves-first từ edges, cycle = 1 unit, unresolved edge = lead không blocker, progress ghi vào `aim_units`) | Đã đạt bằng test (19 test mới, tests/services/test_code_graph_structural.py + test_aim_extractors.py): estate COBOL/JCL mẫu index vào code_nodes/edges qua `reindex_workspace` thật; `search_nodes` (FTS/lexical) tìm thấy paragraph `CALC-PARA` và step `STEP010`; workspace target/kb/non-AIM **không** nhận extractor (role-scoped, chứng minh bằng test file .cbl trong target bị bỏ qua); đo extraction recall trên pilot estate thật vẫn là việc của engagement đầu tiên (risk #1) |
| **AIM-4 — Pipelines** ✅ **ĐÃ IMPLEMENT** (2026-07-16, ba9433c; nền Workflows engine M1-M5 tự build cùng ngày: 24aef4a/2b870eb/a875537/d27a840/c2b72de — definition layer + CRUD/approval + runner headless + agent/gate/input/foreach với team hooks + FE trigger slash/pill; scope enum có `aim` từ M1) | Thư viện 6 workflow YAML (aim-assess/-understand/-convert-unit/-convert-wave/-test-compare/-cutover-check) conform schema v1, builtin discovery root; `aim_units` list thêm `wave_filter`+`format=json` cho tool node; `app/services/aim/rulebook_install.py` install pack workflows/skills vào discovery roots (gap-fill, gọi từ create/join); approval-manifest tự phủ `aim_units`/agents/shell của các node (compute_manifest generic); `GET /team/{sid}/questions/pending` + Gate panel trong AimPipelinesPanel — trigger→gate→result không cần chat; Pipelines chạy `POST /workflows/{name}/run` trên session per-run | **ĐÃ ĐẠT, verified live**: `aim-test-compare core-batch/EODCLOSE smoke` chạy e2e trong session aim — agent compare turn thật → gate certify mang output thật → nhánh triage → completed; `aim-cutover-check wave 0` deterministic e2e: gate "1 of 2 units certified" → foreach ghi `phase: cutover` vào frontmatter KB thật → `{cutover_count: 1}`. Còn lại của Workflows plan: M6 canvas builder (FE-only, không chặn AIM) |
