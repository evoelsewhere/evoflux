...
- The custom no-UAC MSI installs under `LocalAppDataFolder`. WiX 3's ICE38 and
  ICE91 validation rules reject file key paths generated by Tauri for that
  intentionally per-user layout. Tauri does not expose linker arguments, so
  the workflow re-links its generated WiX objects while suppressing only
  those two incompatible ICE rules. Production mode then signs the resulting
  MSI explicitly.
