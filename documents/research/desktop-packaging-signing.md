...
| MSI · Windows x64 | `windows-2022` | x86_64 | `evoflux-windows-x64` |
...
- `auto` (default): use production signing only when every secret required by
  that platform is available. Otherwise build ad-hoc DMGs and an unsigned MSI.
- `required`: fail a platform job immediately when any signing secret is
  missing. Use this for a real public release.
- `off`: always build ad-hoc DMGs and an unsigned MSI.
...
Tauri imports the `.p12`, signs the app and DMG, submits the app to Apple's
notary service, and staples the ticket during `cargo tauri build`. The committed
...
the MSI, and timestamps the signatures.
...
checks the resulting MSI with `Get-AuthenticodeSignature`.
