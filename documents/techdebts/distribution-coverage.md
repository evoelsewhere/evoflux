...
`ghcr.io/khuonghung/evoflux` image is no longer published.

**Why removed:** EvoFlux's threat model assumes a trusted operator
...
app — they fall back to the CLI (`brew install khuonghung/tap/evoflux`) and the
web cockpit.
...
Flatpak's sandbox model conflicts with EvoFlux's "agent reads/writes
