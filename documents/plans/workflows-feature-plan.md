...
All three tiers are **committed scope** (user direction), not a wishlist. Grounded in the builtin-tool inventory (`app/agent/tools/builtin/` — `web`, `browser_use_tool`, `memory_search`, `pr`, `worktree`, `date`, `lsp`, `preview`, `todo`, plus the four anchors) and in patterns proven by n8n/Dify/ComfyUI-class builders.
