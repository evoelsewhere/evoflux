...
| P1 | Graph-navigation preload was attached to built-in Coding members but not guaranteed for the Lead or custom Coding agents. | Agent behavior depended on persona naming rather than Coding-mode policy. | Apply `code-graph-navigation` as a mode-level default for every Coding agent; `skills_opt_out: [code-graph-navigation]` is the explicit override. |
