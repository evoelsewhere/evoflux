...
EvoFlux uses a two-tier logging system built on [loguru](https://github.com/Delgan/loguru): application-wide logs and per-session logs.
...
Log paths live under `{EVOFLUX_STATE_DIR}/logs/`. Value depends on environment:

| Mode | `EVOFLUX_STATE_DIR` |
|------|--------------------|
| Production | `~/.local/state/EvoFlux/` |
| Development | `.EvoFlux/state/` (project root) |
...
In team mode there is one `{agent_name}.jsonl` per active agent (e.g. `EvoFlux.jsonl`, `explorer.jsonl`, `writer.jsonl`); single-agent chat produces one named after the lead agent.
...
under `EVOFLUX_WORKSPACE_DIR`, not `EVOFLUX_STATE_DIR`. See
...
|--------|-----------------|--------------|
| **Location** | `{EVOFLUX_STATE_DIR}/logs/app/app.log` | `{EVOFLUX_STATE_DIR}/logs/sessions/{id}/` |
...
### File Path
```
{EVOFLUX_STATE_DIR}/logs/app/app.log
```
...
with open("{EVOFLUX_STATE_DIR}/logs/app/app.log") as f:
    for line in f:
...
Created dynamically when a chat request arrives. Location: `{EVOFLUX_STATE_DIR}/logs/sessions/{session_id}/`
...
2. Team member activation begins
   - `add_session_sink(session_id)` creates `{EVOFLUX_STATE_DIR}/logs/sessions/{session_id}/` directory, registers loguru sink
...
### Application Log (`{EVOFLUX_STATE_DIR}/logs/app/app.log`)
...
### Session Logs (`{EVOFLUX_STATE_DIR}/logs/sessions/{id}/session.log`)
...
### JSONL Files (`{EVOFLUX_STATE_DIR}/logs/sessions/{id}/{agent}.jsonl`)
...
log_file = Path("{EVOFLUX_STATE_DIR}/logs/app/app.log")
...
event_log = Path(f"{EVOFLUX_STATE_DIR}/logs/sessions/{session_id}/assistant.jsonl")
...
```python
session_id = "019d707b-79ac-735c-894e-001713d09648"
transcript = Path(f"{EVOFLUX_STATE_DIR}/logs/sessions/{session_id}/session.log")
...
# SessionLogHook writes to {EVOFLUX_STATE_DIR}/logs/sessions/abc123/mybot.jsonl
```
...
- File exists: `{EVOFLUX_STATE_DIR}/logs/sessions/{session_id}/session.log`
...
- Clean up stale logs manually if needed:
  ```bash
  find ~/.local/state/EvoFlux/logs -mtime +7 -delete  # production
  find .EvoFlux/state/logs -mtime +7 -delete          # development
  ```
...
|-----------|----------|--------|-------|----------|
| **App Log** | `{EVOFLUX_STATE_DIR}/logs/app/app.log` | JSON | All events | 10 MB / 7 days |
| **Session Transcript** | `{EVOFLUX_STATE_DIR}/logs/sessions/{id}/session.log` | Text | Session only | 5 MB / 3 days |
| **Session Events** | `{EVOFLUX_STATE_DIR}/logs/sessions/{id}/{agent}.jsonl` | JSONL | Per-agent events | None |
