...
**Quick start.** [`install.md`](./install.md) → `EvoFlux init` → `EvoFlux`. Use `EvoFlux start --lan --key` plus `EvoFlux address` for desktop clients on your network.
...
| [Migration](../../MIGRATION.md) | Move setup from OpenClaw, Hermes, Claude Code, Codex CLI, or older EvoFlux installs. |
...
| [Comparison](./comparison.md) | How EvoFlux compares to Claude Code, Codex CLI, Cursor/Windsurf, Aider, opencode. |
