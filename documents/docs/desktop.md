...
description: How EvoFlux packages, ships, signs, and updates the native desktop app.
status: stable
updated: 2026-06-09
---
...
- Generates a URL-safe random token, exposes it on the handshake line,
  and writes it into `EVOFLUX_DESKTOP_TOKEN` for its middleware.
...
| Off-switch                                   | Unset `EVOFLUX_DESKTOP_TOKEN` and any configured access key — the middleware becomes a no-op. CLI / Docker users keep open behaviour unless they enable a key. |
...
The **About EvoFlux** item opens the native About panel populated with the app icon, name, version (from `Cargo.toml` / `tauri.conf.json`), copyright, and a link to the project repository.

**Home**, **Cockpit**, **Coding**, **Settings**, **Providers**, **Notifications**, and **Telemetry** are route shortcuts. **Show EvoFlux** focuses the primary window state; **New Window** (`⌘/Ctrl+N`) opens another cockpit window backed by the same sidecar/API token. **Home** intentionally resets the active webview to the mode picker.
...
The **View → Reload** action (`⌘/Ctrl+R`) calls `window.location.reload()` on the active webview, respecting the HTTP cache. The tray **Reload Window** action uses the same webview-only reload path for cases where the main window is hidden or wedged. **Force Reload** (`⌘/Ctrl+Shift+R`) keeps native windows alive, restarts the managed production sidecar, waits for health, then reinjects the new backend port/token into every open webview. In dev mode (`EVOFLUX_DEV_BACKEND_URL`), Tauri does not own the backend process, so Force Reload falls back to webview-only reload. Reload always brings a window to front before refreshing so the user sees the result.

The utility actions **View Config Folder** and **View Backend Log** are desktop diagnostics. Config opens the shared CLI/desktop config root (`$EVOFLUX_CONFIG_DIR` or `~/.config/evoflux`). Backend log reveals the bundled sidecar's `backend.log` when the sidecar is running; it is unavailable when the app is connected only to an external backend.
...
**Zoom In** / **Zoom Out** / **Actual Size** (`⌘/Ctrl+=`, `⌘/Ctrl+-`, `⌘/Ctrl+0`) drive `Webview::set_zoom` on every open EvoFlux window — the bare `=` key is bound so the user doesn't need Shift, matching Chrome and Safari. The zoom factor multiplies by 1.2 per press, clamped to `[0.5, 3.0]`, and resets to 1.0. State is session-only — not persisted across restarts — because the desktop shell has no other settings store.

Clicking the tray icon opens the tray menu (showing live status first) rather than summoning the main window. The window is summoned explicitly via the "Show EvoFlux" entry. This matches macOS menu-bar app conventions where the icon is a status surface rather than a launcher.
...
Closing the main window hides it to the tray instead of stopping the backend. Closing a secondary window destroys only that window. Selecting **Quit EvoFlux** from the app menu or tray marks the app as quitting, exits Tauri, and lets the existing shutdown path terminate the Python sidecar cleanly.
...
The desktop bundle checks the signed Tauri updater manifest at `https://github.com/khuonghung/evoflux/releases/download/latest-desktop/latest.json`.

- Users can check manually from **EvoFlux → Check for Updates…** or **Settings → About → Updates**.
...
On install, Rust verifies the updater signature, shuts down the Python sidecar, then calls `tauri::process::restart`. CLI/server installs still upgrade via `evoflux upgrade` or the package manager that installed them; see [CLI reference](./cli.md).
...
```
~/Library/Application Support/com.evoflux.desktop/   ← EVOFLUX_DATA_DIR
~/Library/Application Support/com.evoflux.desktop/   ← EVOFLUX_CONFIG_DIR (same root, but app/cli/paths.py keeps logical split)
~/Library/Caches/com.evoflux.desktop/                ← EVOFLUX_CACHE_DIR
~/Library/Logs/com.evoflux.desktop/                  ← EVOFLUX_STATE_DIR + sidecar stderr/stdout
```

### Windows (`EVOFLUX_<ver>_x64-setup.exe`)

```
%LOCALAPPDATA%\EvoFlux\
  EvoFlux.exe
  resources\
    sidecar\
      python\python.exe
...
### Linux (`EVOFLUX_<ver>_amd64.AppImage`, `.deb`)

```
/usr/lib/evoflux/sidecar/python/
/usr/lib/evoflux/sidecar/site-packages/
/usr/bin/evoflux              ← Tauri executable, .deb only
```
...
2. `uv pip install --target` of `EvoFlux` (the local project) into
...
|---|---|---|---|
| `.github/workflows/release.yml` | `workflow_dispatch confirm=release` | ~90 s | `EvoFlux-<ver>-py3-none-any.whl`, `EvoFlux-<ver>.tar.gz`; publishes to PyPI. |
| `.github/workflows/release-desktop.yml` | `workflow_dispatch confirm=release-desktop` | ~20–25 min | `EVOFLUX_<ver>_aarch64.dmg`, `EVOFLUX_<ver>_x64_en-US.msi`, `EVOFLUX_<ver>_amd64.deb`, `latest.json`. |

Both workflows use a **create-or-upload** publish step (`gh release view "$TAG"` → `upload --clobber` if present, else `create`), so order doesn't matter for correctness. The runbook orders them PyPI-first so the canonical auto-generated release notes come from `release.yml`; the desktop matrix then appends its installers ~20 min later. See the operator runbook for release procedures.
...
```
https://github.com/khuonghung/evoflux/releases/download/latest-desktop/latest.json
```

Companion publishers (run automatically off `workflow_run`):

- `.github/workflows/publish-homebrew.yml` — updates `khuonghung/homebrew-tap` `Formula/evoflux.rb` from the PyPI sdist after `release.yml` succeeds.
- `.github/workflows/publish-homebrew-cask.yml` — updates `khuonghung/homebrew-tap` `Casks/evoflux.rb` from the `.dmg` after `release-desktop.yml` succeeds. The resolver waits for a release with an `aarch64.dmg` asset attached, so the formula update and cask update don't race even though they're both triggered off the unified tag.
...
| --------------- | --------------------------------- | ------------------------------------------------ |
| macOS arm64     | `EvoFlux-x.y.z.dmg`            | mount, then `/Volumes/evoflux/install.sh --install` |
| Linux (any)     | `EvoFlux-x.y.z.AppImage` (+ `.deb`, `.rpm`) | `./install.sh --install ./evoflux-x.y.z.AppImage` |
| Windows x64     | `EvoFlux-x.y.z-x64.msi`        | double-click; SmartScreen → *More info → Run anyway* |
...
- **Linux branch** — `chmod +x`, copies to `~/.local/bin/evoflux`,
...
| **1a** | Token auth middleware (`EVOFLUX_DESKTOP_TOKEN`)                                          | ✅ done         |
...
- **Intel macOS**: deferred. `macos-13` runners are deprecating;
  ...
  keep the `uv tool install $1evoflux` path until v2 ships universal2.
