...
- **Desktop/web UI:** open **Settings → Providers**. First launch creates the config/cache/state/data/workspace roots, `{EVOFLUX_CONFIG_DIR}/plugins`, and default editable agents with a placeholder model. Packaged installs download those defaults from the GitHub release seed bundle. API-key providers write to `{EVOFLUX_CONFIG_DIR}/.env`; OAuth providers use the in-app flow and store tokens under `{EVOFLUX_CACHE_DIR}`.
- **CLI/server:** run `EvoFlux init` for first setup, or `EvoFlux auth copilot|codex` for OAuth-only providers.
...
Built-ins are resolved in `app/agent/providers/factory.py`; provider plugins are resolved from `{EVOFLUX_CONFIG_DIR}/plugins/` after built-ins.
...
| `copilot` | `EvoFlux auth copilot` | GitHub Copilot OAuth (device flow). |
| `codex` | `EvoFlux auth codex` | OpenAI Codex via ChatGPT subscription. The UI tries device-code auth first, then falls back to browser PKCE when a workspace disables device-code auth. CLI uses browser PKCE by default; `--device` is available for headless setup. |
...
Each provider card in **Settings → Providers** can save a provider-local `visible_models` list in `{EVOFLUX_CONFIG_DIR}/settings.yaml`:
...
Drop-in provider plugins live in `{EVOFLUX_CONFIG_DIR}/plugins/` and export a `ProviderPlugin` named `provider`. They can:

- appear in **Settings → Providers** with declared credential fields;
- save credentials to `{EVOFLUX_CONFIG_DIR}/.env`;
- store provider-owned OAuth tokens under `{EVOFLUX_CACHE_DIR}/provider-plugins/<provider-id>/`;
...
3. Optional `{EVOFLUX_CONFIG_DIR}/model_registry.yaml` → local override for private or newly released models.
...
Set `EVOFLUX_MODEL_REGISTRY_REFRESH=false` to disable runtime fetches and use only the bundled JSON plus local YAML overlay.
...
```bash
uv run python -m tests.providers.test_bedrock --simple
uv run python -m tests.providers.test_bedrock --tools
```
