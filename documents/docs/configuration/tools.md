...
description: Tools shipped with EvoFlux — filesystem, shell, web, memory.
status: stable
updated: 2026-05-16
---
...
**Source:** `app/agent/tools/builtin/`

Tools below ship with EvoFlux. List only the ones an agent should use under `tools:` in its `.md` frontmatter. Internal API and hook semantics live in [`agent/tools.md`](../agent/tools.md).

## Catalog

### Filesystem

Filesystem tools normally output workspace-relative paths. The `read` tool also accepts absolute paths for the active session's XDG artifact directory and `{EVOFLUX_STATE_DIR}/logs/`, so agents can inspect offloaded tool results and logs. See [`sandbox.md`](./sandbox.md) for path-validation rules.
