...
- Resolve under a denied root: `EVOFLUX_DATA_DIR`, `EVOFLUX_STATE_DIR`, `EVOFLUX_CACHE_DIR`.
  - Exception: `{EVOFLUX_STATE_DIR}/logs/` is readable so agents can inspect EvoFlux logs such as `app/app.log`.
...
```yaml
# {EVOFLUX_CONFIG_DIR}/sandbox.yaml
denied_patterns:
  - "**/.env"
  - "**/.env.*"
  - "**/secrets/**"
