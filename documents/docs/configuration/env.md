...
description: Every env var read by EvoFlux — provider keys, ports, paths, optional extras.
status: stable
updated: 2026-05-16
---
...
| `DATABASE_URL` | `{EVOFLUX_DATA_DIR}/EvoFlux.db` | SQLite path or async DB URL. |
...
|----------|-------------|
| `EVOFLUX_DATA_DIR` | Root for irreplaceable user data (DB). Denied to agent fs tools. |
| `EVOFLUX_CONFIG_DIR` | Root for hand-edited config (agents, skills, prompts, `.env`). |
| `EVOFLUX_STATE_DIR` | Root for logs, telemetry, OTEL rollups, PID file. |
| `EVOFLUX_CACHE_DIR` | Root for regeneratable throwaway data. |
| `EVOFLUX_WORKSPACE_DIR` | Per-session agent workspaces (`{root}/<sid>/`). Allowed by the sandbox. |
| `EVOFLUX_WIKI_DIR` | Wiki memory store (`USER.md`, knowledge dirs, `notes/`). |
| `AGENTS_DIR` | Defaults to `{EVOFLUX_CONFIG_DIR}/agents`. |
| `SKILLS_DIR` | Defaults to `{EVOFLUX_CONFIG_DIR}/skills`. |
| `EVOFLUX_PLUGINS_DIRS` | List of plugin directories separated by the OS path separator (`:` on macOS/Linux, `;` on Windows — same convention as `PATH`/`PYTHONPATH`). Defaults to `{EVOFLUX_CONFIG_DIR}/plugins`. See [`agent/plugins.md`](../agent/plugins.md). |
...
OAuth-based providers don't need an env var — run the matching `EvoFlux auth` command instead:

| Provider | Command |
|----------|---------|
| GitHub Copilot | `EvoFlux auth copilot` |
| OpenAI Codex (ChatGPT) | `EvoFlux auth codex` (or `--device` for SSH) |

Tokens are cached at `{EVOFLUX_CACHE_DIR}/copilot_oauth.json` and `{EVOFLUX_CACHE_DIR}/codex_oauth.json` and refreshed automatically.

## Service-level defaults (not env vars)

Summarization thresholds, tool-result offload sizes, and sandbox limits are module-level constants in their respective service modules — **not** environment variables. Runtime feature toggles such as title generation and Dream live in `{EVOFLUX_CONFIG_DIR}/settings.yaml`; per-agent behavior stays in agent frontmatter.

- `{EVOFLUX_CONFIG_DIR}/settings.yaml` — see [`title-generation.md`](../title-generation.md) for title generation.
