...
**Sources:** `app/api/routes/team/chat.py`, `app/agent/mode/team/`
...
- A team in `{EVOFLUX_CONFIG_DIR}/agents/` (exactly one `role: lead`, at least one `role: member`). `EvoFlux init` seeds a default team.
- At least one provider API key set (or an active OAuth: `EvoFlux auth copilot|codex`).
- Backend running: `make dev` (port `:8000`, reload) or `EvoFlux` (port `:4082`, packaged).
...
Use the API endpoints directly or the test suite for validation. For automated coverage see `tests/agent/mode/team/`.
...
reactivates it so it reports via `team_message`. Use the test suite at `tests/agent/mode/team/` for automated coverage.
...
Uploads land at `{EVOFLUX_WORKSPACE_DIR}/{lead_session_id}/uploads/<uuid>.<ext>` and the agent's filesystem tools see them as `uploads/<filename>`. See [`configuration.md#sandbox`](../configuration.md#sandbox) for path semantics.
...
Edit a member's `.md` file mid-session (e.g. `vim {EVOFLUX_CONFIG_DIR}/agents/explorer.md` and change its `model:`). Send a follow-up message in the same session.
