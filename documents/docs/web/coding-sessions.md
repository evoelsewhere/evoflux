...
- Workspace rows expose **Create worktree**. The backend creates git worktrees under `{EVOFLUX_DATA_DIR}/worktrees/<repo>-<hash>/`, defaults branches to `EvoFlux/<name>`, saves the new worktree as a coding workspace, and opens a fresh session there. EvoFlux-managed worktrees can be removed through the worktree API; arbitrary external worktrees are listed but not deleted by EvoFlux.
