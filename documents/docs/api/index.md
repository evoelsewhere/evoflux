...
agents such as `coding/EvoFlux`. Mutations write the
...
For coding agents, use the path-like API name (`coding/EvoFlux`) while the
frontmatter `name:` stays `EvoFlux`.
...
Manages `{EVOFLUX_CONFIG_DIR}/mcp.json` and the live MCP client runners.
...
`<SERVER>_MCP_CLIENT_SECRET` in `{EVOFLUX_CONFIG_DIR}/.env`, updates the
...
Paths are relative to `EVOFLUX_WIKI_DIR`. See [`agent/memory.md`](../agent/memory.md) for the wiki layout.
...
EvoFlux does not expose speech endpoints. Browser/app voice input is
client-side speech recognition documented in [`web/voice-input.md`](../web/voice-input.md).

## Settings

User-editable runtime settings persisted under `{EVOFLUX_CONFIG_DIR}`. Sandbox patterns match resolved absolute paths for filesystem-tool calls; see [`configuration.md`](../configuration.md#sandbox-model-and-permissions).
...
`PUT` writes `{EVOFLUX_CONFIG_DIR}/sandbox.yaml` atomically. New
...
Tauri updater from **EvoFlux → Check for Updates…** or
**Settings → About → Updates**, and CLI/server users run
`EvoFlux upgrade` in the shell that launched the process.
...
| `mode` | `normal` \| `coding` | `normal` | Coding mode loads agents from `{EVOFLUX_CONFIG_DIR}/agents/coding/` |
...
media/listing, while uploads still live under `EVOFLUX_WORKSPACE_DIR`.

| Endpoint | Source | Scope |
|----------|--------|-------|
| `GET /api/team/{session_id}/uploads/{filename}` | `{EVOFLUX_WORKSPACE_DIR}/{session_id}/uploads/` | User-uploaded attachments (flat, UUID-named) |
...
- `.git/` and common generated directories (`node_modules`, `dist`, `build`,
  ...
  including dot-prefixed ones like `.EvoFlux/`, `.github/`, `.env.example`
  — are surfaced unless filtered by the root `.gitignore`. The picker honours
  `!`-negation, so `.EvoFlux/*` + `!.EvoFlux/skills/` re-includes the
  tracked subtree.
...
`GET /api/team/sessions/{session_id}/todos` returns the session-scoped todo list from `{EVOFLUX_DATA_DIR}/sessions/{session_id}/.todos.json`.
