...
Everything you need to customise EvoFlux. The detailed reference is split across the [`configuration/`](./configuration/) subfolder to keep each page focused.
...
```bash
EvoFlux init        # interactive wizard — picks provider, seeds config
EvoFlux             # start the API server in the background
EvoFlux doctor      # health checks
```

`init` writes config to `~/.config/EvoFlux/` (XDG standard). The database lives at `~/.local/share/EvoFlux/EvoFlux.db`. Logs live at `~/.local/state/EvoFlux/logs/app/app.log`.

Two `.env` files are loaded if present: `~/.config/EvoFlux/.env` takes priority over `.env` in the project root.
...
```bash
EvoFlux                   # start the API server in the background
EvoFlux start --lan --key # expose the server to mobile/LAN clients and save a LAN access key
...
Migrations run automatically on startup. Desktop bundle users update via **EvoFlux → Check for Updates…** in the menu bar; CLI/server users run `EvoFlux upgrade`.
...
- Desktop/mobile clients: use `EvoFlux address` to find the local or LAN URL. In shared networks, run `EvoFlux start --lan --key` once; `server.host`, `server.port`, and `server.access_key` are saved in `settings.yaml` for future restarts.
