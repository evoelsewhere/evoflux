...
# EvoFlux — Developer Guidelines

EvoFlux is an on-machine AI assistant: FastAPI backend, React web UI, multi-agent teams, in-memory SSE streaming.
...
EvoFlux start --lan --key             # expose server to mobile/LAN clients with access-key auth
...
EvoFlux logs                          # tail the server log (alias: EvoFlux logs -n 100)
EvoFlux doctor                        # check system health
EvoFlux upgrade                       # stop, upgrade, and restart if running (desktop app: EvoFlux → Check for Updates…)
EvoFlux --version                     # print version

EvoFlux auth copilot                  # GitHub Copilot OAuth (browser PKCE)
EvoFlux auth codex                    # OpenAI Codex OAuth (browser PKCE — recommended)
EvoFlux auth codex --device           # OpenAI Codex OAuth (headless device-code, SSH-friendly)
```
