...
description: Run opencode-style `!command` shell commands from the EvoFlux composer or API without asking the model to decide on a tool call.
status: stable
updated: 2026-05-31
---
...
EvoFlux dispatches the command directly through the built-in shell tool. The
...
Run the shell command tests via the test suite at `tests/`.
