---
title: Features
description: Canonical, version-cited catalogue of every user-visible EvoFlux feature. Source of truth for slides, README, comparison docs, and marketing copy.
status: stable
updated: 2026-06-16
---
...
The single source of truth for everything EvoFlux does. Every feature lists the
...
> **Headline.** EvoFlux is the desktop cockpit for local AI agents — a
...
**Latest release:** v1.57.1 · June 16, 2026 · [release notes](https://github.com/khuonghung/evoflux/releases/tag/v1.57.1)
...
- **Native app notifications** `[v1.19.0]` — finished assistant turns,
  background tasks, scheduled reminders in the desktop app. Per-session
...
- **Remote-backend mobile shell** *(deprecated, removed)* `[v1.34.0]` — Tauri mobile app scaffold embeds the shared Web UI and connects to saved remote API servers. Removed with the `mobile/` directory.
- **LAN access key for external clients** `[v1.43.0]` — `evoflux start --lan --key`
  stores a `settings.yaml` bearer key so desktop and browser clients need the key before controlling a LAN-exposed server. See [`cli.md`](./cli.md).
...
EvoFlux is multi-agent by default. A lead agent drives the conversation and
...
- **Coding team variant** `[since v1.0]` — `agents/coding/` ships a separate
  compact team (`coding/evoflux`, `coding/coder`, `coding/explorer`) tuned for
  workspace-aware sessions; the coding explorer focuses on inspecting the current
  codebase before implementation.
- **Built-in first-party agent profiles** `[v1.23.0]` — the default `EvoFlux`
...
- **Git worktree sessions** `[v1.41.0]` — create an isolated git worktree from
  ...
  list existing worktrees, and remove EvoFlux-managed worktrees.
...
- **Slash commands scoped to coding workspaces** `[v1.17.0]` — project-local
  commands in `.evoflux/commands/**/*.md` and `.opencode/commands/**/*.md`
...
- **Snippet picker** `[v1.31.0]` — in coding workspaces, type `#` anywhere
  in the composer to pick prompt snippets from `.evoflux/snippets/**/*.md`
  or `{EVOFLUX_CONFIG_DIR}/snippets/**/*.md` and insert the rendered body.
...
EvoFlux carries durable, editable memory across sessions. Deeper doc:
...
- **Editable wiki memory** `[since v1.0]` — Karpathy-style wiki at
  `{EVOFLUX_WIKI_DIR}` with `sources/`, `topics/`, `entities/`,
...
| GitHub Copilot (OAuth) | `copilot:gpt-5.4-mini` | `evoflux auth copilot` |
| OpenAI Codex (OAuth) | `codex:gpt-5.5` | `evoflux auth codex` |
...
- **Sandboxed UI artifacts** `[v1.36.0]` *(beta)* — tool-produced HTML UI
  ...
  Production desktop Tauri shells allow `about:` frames so `srcdoc`
...
- **Plugins** `[v1.6.0]` — Python files dropped into `EVOFLUX_PLUGINS_DIRS`.
...
- **Path denylist** `[since v1.0]` — absolute paths anywhere on disk are accepted
  *unless* they resolve under a denied root (`EVOFLUX_DATA_DIR`,
  `EVOFLUX_STATE_DIR`, `EVOFLUX_CACHE_DIR`) or match a user-defined glob
...
Client-side speech recognition. EvoFlux does not run backend microphone transcription.
...
- **macOS desktop** `[since v1.0]` — Homebrew cask
  (`brew install --cask khuonghung/tap/evoflux`) or `.dmg` with bundled
...
- **CLI install** `[since v1.0]` — `uv tool install $1evoflux`, `pipx`, `pip`,
  `brew install khuonghung/tap/evoflux`.
- **CLI server control** `[v1.41.0]` — `restart`, `address`, `health`, and
  `start --lan --key` make the CLI the control plane for desktop backends.
- **CLI upgrade** `[v1.41.0]` — `evoflux upgrade` stops the background
...
- **Docker** *(deprecated, removed in v1.23.0)* — the `Dockerfile`,
  `docker-compose.yaml`, and the `ghcr.io/khuonghung/evoflux` image are
  no longer maintained. Use the CLI install paths above; revisit if there
  is concrete self-hoster demand.
- **Migration imports** `[since v1.0]` — `evoflux migrate openclaw`,
...
The same HTTP + SSE API drives the desktop and browser clients. Embed it elsewhere with no extra work. Deeper doc: [`api/index.md`](./api/index.md).
...
This document is the **canonical** answer to "what does EvoFlux do?". Slides,
