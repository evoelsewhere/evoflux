...
File: {EVOFLUX_DATA_DIR}/sessions/{session_id}/.tool_results/assistant/{tool_call_id}.txt
...
Writes verbose structured JSONL to `{EVOFLUX_STATE_DIR}/logs/sessions/{session_id}/{agent}.jsonl` — see [`logging.md`](../logging.md).
...
| `Agent._load_plugin_hooks(role)` | Lazily loads user-authored plugin hooks from `EVOFLUX_PLUGINS_DIRS` once per `(agent, role)` — see [`plugins.md`](./plugins.md) |
