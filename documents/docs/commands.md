...
|---|------|--------------|------------|
| 1 | `{workspace}/.EvoFlux/commands/` | `project-EvoFlux` | Project-specific, EvoFlux-native |
| 2 | `{workspace}/.opencode/commands/`   | `project-opencode`   | Reuse an opencode project library |
| 3 | `{EVOFLUX_CONFIG_DIR}/commands/` | `global-EvoFlux` | Your personal library |
...
- **Built-in prompt commands** (`/init`, `/loop <prompt>`, `/loop:{subcommand}`) are handled by
  EvoFlux. `/init` renders through the backend command endpoint. `/loop:*`
...
`~/.config/EvoFlux/commands/git/commit.md`:
...
EvoFlux sends the exact configured prompt as a user message. Each time the
