...
# EvoFlux Architecture (C4 Model)

This document provides a detailed technical overview of **EvoFlux** using the C4 model.

## 1. System Context Diagram (Level 1)
The highest level of abstraction, showing EvoFlux in its environment.

```mermaid
C4Context
    title System Context Diagram for EvoFlux

    Person(user, "User", "Operator on the same machine. Drives the agent via desktop app or web UI.")
    System(EvoFlux, "EvoFlux", "Local on-machine AI assistant. Desktop app (Tauri) embedding the web UI + FastAPI API sidecar. Multi-agent teams, persistent memory, web cockpit UI.")
    ...
    Rel(user, EvoFlux, "Sends messages, receives SSE stream", "HTTP/SSE")
    Rel(EvoFlux, llm_providers, "Sends prompts, receives completion/tools", "HTTPS/SSE")
    Rel(EvoFlux, web, "Searches and fetches content", "HTTPS")
```

---

## 2. Container Diagram (Level 2)
Zooming into the EvoFlux system to see its internal containers.

```mermaid
C4Container
    title Container Diagram for EvoFlux

    Person(user, "User", "Browser")

    System_Boundary(EVOFLUX_boundary, "EvoFlux System") {
        ...
        Container(web, "Web Frontend", "React / TypeScript / Vite / Bun", "Shared desktop/browser UI. Command palette, slash commands, tool inspector with diffs, file panel, telemetry dashboard, multi-agent split view. Connects to backend via REST + SSE.")
...
```mermaid
C4Component
    title Component Diagram for EvoFlux FastAPI Application
    ...
    Container_Boundary(api_boundary, "FastAPI Application") {
        ...
        Component(plugins, "Plugins", "app/agent/plugins/", "User-authored .py drop-ins loaded from settings.EVOFLUX_PLUGINS_DIRS. Loader resolves @plugin functions and Plugin(BaseAgentHook) classes per (agent, role).")
...
under `{EVOFLUX_STATE_DIR}/logs/`. See [`logging.md`](./logging.md) for the
...
EvoFlux is a **single-user, local-first** application. The security model assumes:
