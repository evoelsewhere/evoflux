...
description: Component guidelines for EvoFlux UI — agent chips, sidebar, input bar, tool calls, theme toggle
status: stable
updated: 2026-05-09
---
...
How EvoFlux's component language is built and combined. Each section states the role of the component, the surface and motion rules it must follow, the states it can express, and what to avoid. Pair this page with [colors.md](./colors.md), [interaction.md](./interaction.md), [motion.md](./motion.md), and [layout.md](./layout.md) — those define the underlying tokens and behaviors this page composes.

Every component below works in both light and dark modes without per-mode overrides; brand pigment is reserved for the EvoFlux mascot; agent identity is carried by chips and dots, never by chrome.
...
- Reuse a chip color outside its role (no mint anywhere except `EvoFlux`).
...
A card separates a self-contained unit of content from the surrounding page. The EvoFlux card is a calm paper rectangle — warm fill, hairline border, generous corner radius — and that is it.
...
Empty surfaces — no sessions, no tasks, no files, no agents matched a search — reuse the same hand-drawn idiom: a short Inter callout in `--color-text-subtle`, sometimes paired with the EvoFlux mascot. The voice is conversational, not instructional.

**Rules**

- Inter at the hand size from [typography.md § Hierarchy](./typography.md#type-hierarchy). Centered, with line breaks shaped to read like written speech.
- Color is `--color-text-subtle` so the callout sits one layer behind the surrounding chrome.
- Inter is decorative; pair it with an accessible Inter equivalent in the DOM, or mark the Inter `aria-hidden` and lift the meaning into a sibling element.
...
The home screen's empty state is the most prominent example: mascot at a moderate size, with the Inter prompt "what's on your mind?" below it. This is the one place the prompt is allowed; do not reuse the exact phrase elsewhere.

**Don't**

- Use Inter for instructional copy ("Click + to add a task"). Instructions belong in Inter.
...
- Title: Inter at 28px, bold, `--color-text`. The wordmark is decorative chrome — it is also conveyed by the document title and by the logo asset itself, so a screen reader will still encounter "EvoFlux" elsewhere.
...
- Body text is Inter; code text is JetBrains Mono; Inter is opt-in only on hand callouts.
...
- Brand pigment (EvoFlux) is on the mascot and lockups only — never on UI chrome.
- Motion durations and easings come from [motion.md](./motion.md); `prefers-reduced-motion` is honored.
- Font-weight transitions are on interactive elements only; never on Inter.
