...
The interaction language defines how the EvoFlux UI responds to pointer, keyboard, and touch input. It consumes tokens from [motion.md](./motion.md) and [typography.md](./typography.md) and applies them consistently across every component.
...
Every interactive element moves through a subset of these states. The transitions between them are where the EvoFlux product identity lives.
...
A signature of the EvoFlux interaction language. Text shifts weight under the cursor — subtly, without moving. See [typography.md](./typography.md#font-weight-transitions-signature-interaction) for the full rule.
...
EvoFlux does not rely on custom gestures (swipe, pinch, etc.) for core functionality. Platform-standard gestures work — back-swipe on iOS, pull-to-refresh on Android — and they are respected, not overridden.
...
| **Weight shift on Inter** | Hand-drawn weights aren't perceptually distinct the way Inter weights are. |
| **Agent chip color used outside its role** | Chip colors are role-identity-reserved — using mint anywhere that isn't the EvoFlux role breaks the language. |
