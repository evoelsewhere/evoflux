...
description: 4px grid, breakpoints, radius scale, depth on minimal monochrome surfaces, focus rings, accessibility
status: stable
updated: 2026-05-09
---
...
Both modes use a subtle `box-shadow` plus a `border` to create hierarchy. Unlike a cool zinc theme where brightness steps alone could carry depth, the minimal monochrome palette compresses surface differences too tightly for brightness to do the work — `--bg-page` (`#FFFFFF`) and `--bg-card` (`#FFFBF1`) are only one shade apart by design. The shadow restores the layer separation; the border keeps the card's edge crisp.
...
All EvoFlux tokens are pre-verified — see [colors.md](./colors.md#accessibility-wcag-21-aa).
